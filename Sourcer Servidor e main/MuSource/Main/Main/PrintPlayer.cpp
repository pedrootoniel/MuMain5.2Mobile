...
void InitPrintPlayer() // OK
{
	...
	SetCompleteHook(0xE8,0x006A2EC6,&RenderNumber);
	...
	SetCompleteHook(0xE8,0x006D5E59,&PrintPlayerLevel);

	SetCompleteHook(0xE8,0x006D5E9B,&PrintPlayerLevel);

	SetCompleteHook(0xE8,0x007172E6,&PrintPlayerViewPcPoints);

	SetCompleteHook(0xE8,0x00735CBF,&PrintPlayerViewPcPoints);

	SetCompleteHook(0xE8,0x006011DE,&PrintPlayerMapName);

	SetCompleteHook(0xE8,0x006045AD,&PrintPlayerMapName);

	SetCompleteHook(0xE9,0x0050A075,&CalculateAttackSpeed);
}

void RenderNumber(float* a, int b, float* c, float d, float e) // OK
{
	if(b != -1 && ((bool(__cdecl*)(DWORD))0x0048DBE0)(*(DWORD*)MAIN_CURRENT_MAP) == true)
	{
		b = -2;
		e = 1.0f;
	}

	((void(*)(float*,int,float*,float,float))0x005E0456)(a,b,c,d,e);
}
...
void PrintPlayerLevel(char* a, char* b, char* c) // OK
{
	wsprintf(a,gCustomMessage.GetMessage(40),ViewLevel,ViewReset);
}

void PrintPlayerViewPcPoints(char* a,char* b,char* c) // OK
{
...
void CalculateAttackSpeed(DWORD This)
{
	...
	CharacterAttribute->Shield = GET_MAX_WORD_VALUE(ViewCurBP);
	CharacterAttribute->ShieldMax = GET_MAX_WORD_VALUE(ViewMaxBP);
	...
	SKILL_ATTACK_SPEED_INFO* lpInfo = gSkillAttackSpeed.GetInfo(*(WORD*)(*(DWORD*)MAIN_VIEWPORT_STRUCT+38));
	...
	SetByte(0x004D5DC7,AnimationValue);
