...
void ReconnectMainProc() // OK
{
	...
	if((GetTickCount()-ReconnectMaxTime) > ReconnectMaxWait)
	{
		...
		((void(__thiscall*)(void*))0x00636E30)((void*)MAIN_ACTIVE_SOCKET);
		return;
	}
...
void ReconnectDrawInterface() // OK
{
	...
	pDrawText(pTextThis(),320, 91, gCustomMessage.GetMessage(26+ReconnectProgress), 0, 0, 8, 0);
...
void ReconnectViewportDestroy() // OK
{
	...
	do
	{
		...
		{
			if((ViewportAddress=(pViewportAddress+(count*0x464))) == 0)
			{
				continue;
			}
