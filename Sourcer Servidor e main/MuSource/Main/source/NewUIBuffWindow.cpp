...
void SEASON3B::CNewUIBuffWindow::RenderBuffIcon(eBuffState& eBuffType, float x, float y, float width, float height)
{
	...
	if (eBuffType < 81) // eBuff_Berserker
	{
		...
		RenderBitmap(IMAGE_BUFF_STATUS, x, y, width - 5, height - 5, u, v, width/256.f, height/256.f);
	}
	else
	{
		// eBuff_Santa
		...
		RenderBitmap(IMAGE_BUFF_STATUS2, x, y, width - 5, height - 5, u, v, width/256.f, height/256.f);
	}
}
