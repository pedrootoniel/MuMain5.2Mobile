...
{
	LoadBitmap("Common\\Interface\\ui_chat_back_left.jpg", IMAGE_INPUTBOX_BACK_LEFT);
	LoadBitmap("Common\\Interface\\ui_chat_back_right.jpg", IMAGE_INPUTBOX_BACK_RIGHT);
	LoadBitmap("Common\\Interface\\ui_chat_btn_whisper.jpg", IMAGE_INPUTBOX_BTN_WHISPER);

	/*LoadBitmap("Interface\\newui_chat_normal_on.jpg", IMAGE_INPUTBOX_NORMAL_ON);
...
void SEASON3B::CNewUIChatInputBox::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_INPUTBOX_BTN_WHISPER);
	DeleteBitmap(IMAGE_INPUTBOX_BACK_LEFT);
	DeleteBitmap(IMAGE_INPUTBOX_BACK_RIGHT);
}
...
bool SEASON3B::CNewUIChatInputBox::Create(CNewUIManager* pNewUIMng, CNewUIChatLogWindow* pNewUIChatLogWnd, int x, int y)
{
	...
	m_pChatInputBox->SetPosition(m_WndPos.x + 14, m_WndPos.y + 32);
	...
	m_pWhsprIDInputBox->SetPosition(m_WndPos.x + 198, m_WndPos.y + 32);
...
bool SEASON3B::CNewUIChatInputBox::UpdateMouseEvent()
{
	...
	if (CheckMouseIn(m_WndPos.x + 254, m_WndPos.y + 28, 15, 16) == true && SEASON3B::IsRelease(VK_LBUTTON)) // OK
	{
...
bool SEASON3B::CNewUIChatInputBox::Render()
{
	...
	if (m_bWhisperSend == false)
	{
		...
		RenderColor(m_WndPos.x + 196, m_WndPos.y + 28, 64, 17);
...
{
	RenderBitmap(IMAGE_INPUTBOX_BACK_LEFT, m_WndPos.x + 7.f, m_WndPos.y + 23.f, (WND_WIDTH / 2), WND_HEIGHT, 0, 0, 0.51953125, 0.90625, 1, 1, 0.0);
	RenderBitmap(IMAGE_INPUTBOX_BACK_RIGHT, m_WndPos.x + 141.f, m_WndPos.y + 23.f, (WND_WIDTH / 2), WND_HEIGHT, 0, 0, 0.51953125, 0.90625, 1, 1, 0.0);

	if (m_bWhisperSend == true)
	{
		RenderBitmap(IMAGE_INPUTBOX_BTN_WHISPER, m_WndPos.x + 254.f, m_WndPos.y + 28.f, 15.0, 16.0, 0, 0, 0.9375, 1.0, 1, 1, 0.0);
	}
