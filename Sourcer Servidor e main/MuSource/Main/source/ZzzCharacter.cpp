...
void SetPlayerStop(CHARACTER *c)
{
	...
	if(c->Object.Type==MODEL_PLAYER)
	{
		...
		else if (c->Weapon[1].Type == MODEL_HELPER + 5 && c->SafeZone && !gMapManager.InChaosCastle())
        {
...
void SetCharacterScale(CHARACTER *c)
{
	...
	if (c->BodyPart[BODYPART_HELM].Type == MODEL_HELM || c->BodyPart[BODYPART_HELM].Type == MODEL_HELM + 2 || (c->BodyPart[BODYPART_HELM].Type >= MODEL_HELM + 10 && c->BodyPart[BODYPART_HELM].Type <= MODEL_HELM + 13))
	{
