...
void SEASON3B::CNewUIMessageBoxButton::Render()
{
	...
	if (m_dwSizeType == MSGBOX_BTN_SIZE_OK)
	{
		if (bBtnHover)
		{
			RenderBitmap((m_dwTexType + 1), m_x, m_y, m_width, m_height, 0.f, 0, 0.546875, 0.65625, 1, 1, 0.0);
		}
		else
		{
			RenderBitmap(m_dwTexType, m_x, m_y, m_width, m_height, 0.f, 0, 0.546875, 0.65625, 1, 1, 0.0);
		}
	}
	else if (m_dwSizeType == MSGBOX_BTN_SIZE_EMPTY)
	{
		...
		RenderBitmap(m_dwTexType, m_x, m_y, m_width, m_height, 0.f, 0.f, 0.83203125, 1.0, 1, 1, 0.0);
	...
	else if (m_dwSizeType == MSGBOX_BTN_SIZE_EMPTY_SMALL)
	{
		...
		RenderBitmap(m_dwTexType, m_x, m_y, m_width, m_height, 0, 0, 0.765625f, 1.0, 1, 1, 0.0);
	...
	if(m_strText.size() > 0)
	{
		...
		if (m_dwSizeType == MSGBOX_BTN_SIZE_EMPTY_SMALL)
		{
			g_pRenderText->RenderText(x, m_y + 5, m_strText.c_str());
		}
		else
		{
...
void SEASON3B::CNewUICommonMessageBox::RenderFrame()
{
	...
	else
	{
		...
		width = MSGBOX_WIDTH; height = MSGBOX_BOTTOM_HEIGHT;
		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BACK, x, y + MSGBOX_LINE_HEIGHT, width, height, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);
		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BACK, x, y + 45, width, height, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);

		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_LINE, x, y + 90, MSGBOX_WIDTH, MSGBOX_LINE_HEIGHT, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);
	}
}	
...
void SEASON3B::CNewUI3DItemCommonMsgBox::SetButtonInfo()
{
	...
	case MSGBOX_COMMON_TYPE_OKCANCEL:
		x = GetPos().x + (((GetSize().cx / 2) - MSGBOX_BTN_WIDTH) / 2);
		y = GetPos().y + GetSize().cy - (MSGBOX_BTN_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK) - 5;
		width = MSGBOX_BTN_WIDTH;
		height = MSGBOX_BTN_HEIGHT;
		m_BtnOk.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_OK, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_OK);
...
void SEASON3B::CNewUI3DItemCommonMsgBox::RenderFrame() // -> Shop
{
	float x, y, width, height;
