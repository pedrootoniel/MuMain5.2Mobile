...
void SEASON3B::CNewUIMoveCommandWindow::SetPos(int x, int y)
{
	...
	case 800:
		...
		break;
	case 1024:
		m_MapNameUISize.x = 180; m_MapNamePos.x = m_Pos.x + 54; m_ReqLevelPos.x = m_Pos.x + 119; m_ReqZenPos.x = m_Pos.x + 159;
		break;
	case 1280:
		m_MapNameUISize.x = 160; m_MapNamePos.x = m_Pos.x + 49; m_ReqLevelPos.x = m_Pos.x + 104; m_ReqZenPos.x = m_Pos.x + 139;
		break;
	case 1366:
		m_MapNameUISize.x = 150; m_MapNamePos.x = m_Pos.x + 46; m_ReqLevelPos.x = m_Pos.x + 101; m_ReqZenPos.x = m_Pos.x + 134;
		break;
	case 1440:
		m_MapNameUISize.x = 140; m_MapNamePos.x = m_Pos.x + 43; m_ReqLevelPos.x = m_Pos.x + 97; m_ReqZenPos.x = m_Pos.x + 129;
		break;
	case 1600:
		m_MapNameUISize.x = 120; m_MapNamePos.x = m_Pos.x + 36; m_ReqLevelPos.x = m_Pos.x + 86; m_ReqZenPos.x = m_Pos.x + 114;
		break;
	case 1680:
		m_MapNameUISize.x = 115; m_MapNamePos.x = m_Pos.x + 34; m_ReqLevelPos.x = m_Pos.x + 83; m_ReqZenPos.x = m_Pos.x + 110;
		break;
	case 1920:
		m_MapNameUISize.x = 110; m_MapNamePos.x = m_Pos.x + 28; m_ReqLevelPos.x = m_Pos.x + 70; m_ReqZenPos.x = m_Pos.x + 93;
		break;
	}
...
void SEASON3B::CNewUIMoveCommandWindow::SettingCanMoveMap()
{
	...
	for( int i=0 ; i<m_iRenderEndTextIndex ; i++, li++ )
	{
		...
		if ((gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_DARK || gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_DARK_LORD) && (iReqLevel != 400))
		{
	...
	for ( int i=0 ; i<m_iRenderEndTextIndex ; i++, li++ )
    {
		...
		if( (*li)->_bCanMove == true )
		{
			...
			if( (*li)->_bSelected == true )
			{
				...
				RenderColor( iX, iY-1, m_MapNameUISize.x - 5, m_iRealFontHeight );
				glColor4f ( 1.f, 1.f, 1.f, 1.f );
				EnableAlphaTest();
			}
		}
