...
void SEASON3B::CNewUIChatLogWindow::Init()
{
	...
	this->m_iMessagePosY = 280;
}

void SEASON3B::CNewUIChatLogWindow::LoadImages()
{
	LoadBitmap("Common\\Interface\\ui_msgbox_line.jpg", IMAGE_SCROLL_TOP);

	LoadBitmap("Common\\Interface\\ui_chat_nis_rsframe.tga", IMAGE_SCROLL_NIS_RSFRAME);
	LoadBitmap("Common\\Interface\\ui_chat_nis_btn_arrow.jpg", IMAGE_SCROLL_NIS_BTN_ARROW);
	LoadBitmap("Common\\Interface\\ui_chat_nis_vframe.jpg", IMAGE_SCROLL_NIS_VFRAME);
	LoadBitmap("Common\\Interface\\ui_chat_nis_back.jpg", IMAGE_SCROLL_NIS_BACK);
	LoadBitmap("Common\\Interface\\ui_chat_nis_bar.jpg", IMAGE_SCROLL_NIS_BAR);
...
void SEASON3B::CNewUIChatLogWindow::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_SCROLL_NIS_RSFRAME);
	DeleteBitmap(IMAGE_SCROLL_NIS_BTN_ARROW);
	DeleteBitmap(IMAGE_SCROLL_NIS_VFRAME);
	DeleteBitmap(IMAGE_SCROLL_NIS_BACK);
	DeleteBitmap(IMAGE_SCROLL_NIS_BAR);
...
{
	float fRenderPosX = m_WndPos.x, fRenderPosY = (m_WndPos.y - m_WndSize.cy + SCROLL_TOP_BOTTOM_PART_HEIGHT) - this->m_iMessagePosY;
	...
	if (m_bShowFrame)
	{
		...
		fRenderPosX = m_WndPos.x + 195;

		fRenderPosY = (406 - m_WndSize.cy);
	}
...
bool SEASON3B::CNewUIChatLogWindow::RenderBackground()
{
	if (m_bShowFrame)
	{
		float fRenderPosX = m_WndPos.x + 187, fRenderPosY = (406 - m_WndSize.cy);

		EnableAlphaTest();
		glColor4f(0.0f, 0.0f, 0.0f, GetBackAlpha());
		RenderColor(fRenderPosX, fRenderPosY, (float)m_WndSize.cx, (float)m_WndSize.cy + 2);
...
bool SEASON3B::CNewUIChatLogWindow::RenderFrame()
{
	if(m_bShowFrame)
	{
		...
		RenderBitmap(IMAGE_SCROLL_TOP, fRenderPosX, fRenderPosY - float(5), float(WND_WIDTH), 5.0, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);

		/* */

		RenderBitmap(IMAGE_SCROLL_NIS_RSFRAME, fRenderPosX + 240, fRenderPosY - float(9), 22.0, 12.0, 0, 0, 0.6875, 0.75, 1, 1, 0.0);
		...
		{	
			RenderBitmap(IMAGE_SCROLL_NIS_VFRAME, fRenderPosX, fRenderPosY - 3 + (float)(i * 40 + SCROLL_TOP_BOTTOM_PART_HEIGHT), 5.0, 40.0, 0, 0, 0.625, 0.625, 1, 1, 0.0);
		
			RenderBitmap(IMAGE_SCROLL_NIS_VFRAME, (fRenderPosX + WND_WIDTH) - 5, fRenderPosY - 3 + (float)(i * 40 + SCROLL_TOP_BOTTOM_PART_HEIGHT), 5.0, 40.0, 0, 0, 0.625, 0.625, 1, 1, 0.0);
		}
		...
		RenderBitmap(IMAGE_SCROLL_NIS_BTN_ARROW, (fRenderPosX + WND_WIDTH) - 20, fRenderPosY + 2, 13.0, 13.0, 0, 0, 0.8125, 0.8125, 1, 1, 0.0);

		for (int i = 0; i < (int)GetNumberOfShowingLines(); i++)
		{
			RenderBitmap(IMAGE_SCROLL_NIS_BACK, (fRenderPosX + WND_WIDTH) - 20, fRenderPosY + 15 + (float)(i * 15.0), 13.0, 15.0, 0, 0, 0.8125, 0.8125, 1, 1, 0.0);

			RenderBitmap(IMAGE_SCROLL_NIS_BAR, (fRenderPosX + WND_WIDTH) - 19, fRenderPosY + 15 + (float)(i * 15.0), 13.0, 15.0, 0, 0, 0.8125, 0.8125, 1, 1, 0.0);
		}

		RenderBitmap(IMAGE_SCROLL_NIS_BTN_ARROW, (fRenderPosX + WND_WIDTH) - 20, m_WndPos.y + 25, 13.0, -13.0, 0, 0, 0.8125, 0.8125, 1, 1, 0.0);
		...
		//RenderImage(IMAGE_SCROLLBAR_ON, m_ScrollBtnPos.x, m_ScrollBtnPos.y, SCROLL_BTN_WIDTH, SCROLL_BTN_HEIGHT); Scrooll
...
bool SEASON3B::CNewUIChatLogWindow::UpdateMouseEvent()
{
	...
	int PosX = m_WndPos.x;
	int PosY = m_WndPos.y - this->m_iMessagePosY - 60;

	if (m_bShowFrame) 
	{
		PosX = m_WndPos.x + 195;
		PosY = (406 - m_WndSize.cy);
	}

	if (m_EventState == EVENT_NONE && false == MouseLButtonPush && SEASON3B::CheckMouseIn(PosX, PosY, m_WndSize.cx, m_WndSize.cy))
	{
		m_EventState = EVENT_CLIENT_WND_HOVER;
		return false;
	}

	if (false == MouseLButtonPush && m_EventState == EVENT_CLIENT_WND_HOVER && false == SEASON3B::CheckMouseIn(PosX, PosY, m_WndSize.cx, m_WndSize.cy))
	{
		m_EventState = EVENT_NONE;
		return true;
	}
	...
	if(SEASON3B::CheckMouseIn(PosX, PosY, m_WndSize.cx, m_WndSize.cy))
	{
		int iRenderStartLine = 0;
		if(GetCurrentRenderEndLine() >= m_nShowingLines)
			iRenderStartLine = GetCurrentRenderEndLine() - m_nShowingLines + 1;
		...
		for(int i=iRenderStartLine, s=0; i<=GetCurrentRenderEndLine(); i++, s++)
		{
			...
			{
				float fRenderPosX = PosX;
				float fRenderPosY = PosY + SCROLL_TOP_BOTTOM_PART_HEIGHT;
	...
	if(m_bShowFrame)
	{
		...
		if(m_EventState == EVENT_SCROLL_BTN_DOWN)
		{
			if(SEASON3B::IsRepeat(VK_LBUTTON))
			{
				...
				{
					extern int MouseY;
					if(MouseY-m_iGrapRelativePosY < PosY +WND_TOP_BOTTOM_EDGE)
					{
						Scrolling(GetNumberOfShowingLines()-1);
						m_ScrollBtnPos.y = PosY+WND_TOP_BOTTOM_EDGE;
					}
					else if(MouseY-m_iGrapRelativePosY > PosY)
					{
						Scrolling(GetNumberOfLines(GetCurrentMsgType())-1);
						m_ScrollBtnPos.y = PosY;
					}
					else
					{
						float fScrollRate = (float)((MouseY-m_iGrapRelativePosY) - PosY) / (float)(m_WndSize.cy-WND_TOP_BOTTOM_EDGE*2-SCROLL_BTN_HEIGHT);
		...
		POINT ptResizingBtn = { m_WndPos.x + 427, (397 - m_WndSize.cy) };
		...
		if(m_EventState == EVENT_RESIZING_BTN_DOWN)
		{
			if(MouseLButtonPush)
			{
				...
				{
					if (SEASON3B::CheckMouseIn(0, ptResizingBtn.y - RESIZING_BTN_HEIGHT - ((i + 1) * SCROLL_MIDDLE_PART_HEIGHT * 3 * 2), 640, SCROLL_MIDDLE_PART_HEIGHT * 3 + RESIZING_BTN_HEIGHT))
					{
				...
				{
					if (SEASON3B::CheckMouseIn(0, ptResizingBtn.y + RESIZING_BTN_HEIGHT + ((i + 1) * SCROLL_MIDDLE_PART_HEIGHT * 3), 640, RESIZING_BTN_HEIGHT + SCROLL_MIDDLE_PART_HEIGHT * 3))
					{
						SetNumberOfShowingLines(GetNumberOfShowingLines() - (i + 1) * 3);
						return false;
					}
				...
				if (SEASON3B::CheckMouseIn(0, m_WndPos.y - (SCROLL_MIDDLE_PART_HEIGHT * 3 + SCROLL_TOP_BOTTOM_PART_HEIGHT * 2), 640, SCROLL_MIDDLE_PART_HEIGHT * 3 + SCROLL_TOP_BOTTOM_PART_HEIGHT * 2))
				{
