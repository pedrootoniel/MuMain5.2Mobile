...
bool CNewUIStorageInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if (false == m_pNewInventoryCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, x + 15, y + 36, 8, 15))
	{
...
bool CNewUIStorageInventory::Render()
{
	...
	if (SEASON3B::CheckMouseIn(m_Pos.x + 38, m_Pos.y + 390, 24, 24) == true)
	{
		RenderTipText(m_Pos.x + 38, m_Pos.y + 375, GlobalText[235]);
	}

	RenderBitmap(IMAGE_STORAGE_BTN_ZEN, m_Pos.x + 38, m_Pos.y + 390, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);

	if (SEASON3B::CheckMouseIn(m_Pos.x + 78, m_Pos.y + 390, 24, 24) == true)
	{
		RenderTipText(m_Pos.x + 78, m_Pos.y + 375, GlobalText[236]);
	}

	RenderBitmap(IMAGE_STORAGE_BTN_ZEN, m_Pos.x + 78, m_Pos.y + 390, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);

	if (SEASON3B::CheckMouseIn(m_Pos.x + 118, m_Pos.y + 390, 24, 24) == true)
	{
		RenderTipText(m_Pos.x + 118, m_Pos.y + 375, GlobalText[242]);
	}

	if (this->m_bLock)
	{
		RenderBitmap(IMAGE_STORAGE_BTN_UNLOCK, m_Pos.x + 118, m_Pos.y + 390, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);
	}
	else
	{
		RenderBitmap(IMAGE_STORAGE_BTN_LOCK, m_Pos.x + 118, m_Pos.y + 390, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);
	}
...
{
	RenderBitmap(IMAGE_STORAGE_TOP, m_Pos.x, m_Pos.y, 190.0, 256.0, 0, 0, 0.7421875, 1.0, 1, 1, 0.0);
	RenderBitmap(IMAGE_STORAGE_LOW, m_Pos.x, m_Pos.y + 256.0, 190.0, 177.0, 0, 0, 0.7421875, 0.69140625, 1, 1, 0.0);

	RenderBitmap(IMAGE_STORAGE_MONEY, m_Pos.x + 55, m_Pos.y + 342, 110.0, 18.0, 0, 0, 0.859375, 0.5625, 1, 1, 0.0);
}
...
void CNewUIStorageInventory::RenderText()
{
	...
	g_pRenderText->RenderText(m_Pos.x + 65, m_Pos.y + 345, szTemp, 0, 0, RT3_SORT_RIGHT);

	g_pRenderText->SetBgColor(30, 30, 30, 255);
	g_pRenderText->SetTextColor(240, 64, 64, 255);
	g_pRenderText->RenderText(m_Pos.x + 55, m_Pos.y + 368, GlobalText[266]);

	__int64 iTotalLevel = (__int64)CharacterAttribute->Level;
	...
	g_pRenderText->RenderText(m_Pos.x + 120, m_Pos.y + 368, szTemp, 0, 0, RT3_SORT_RIGHT);

	g_pRenderText->RenderText(m_Pos.x + 32, m_Pos.y + 345, GlobalText[100]);
}
...
{
	LoadBitmap("Common\\Interface\\ui_item_backtop.jpg", IMAGE_STORAGE_TOP);
	LoadBitmap("Common\\Interface\\ui_item_backlow.jpg", IMAGE_STORAGE_LOW);

	LoadBitmap("Common\\Interface\\ui_ware_money.jpg", IMAGE_STORAGE_BTN_ZEN);
	LoadBitmap("Common\\Interface\\ui_ware_money_click.jpg", IMAGE_STORAGE_BTN_ZEN_CLICK);

	LoadBitmap("Common\\Interface\\ui_ware_locked.jpg", IMAGE_STORAGE_BTN_UNLOCK);
	LoadBitmap("Common\\Interface\\ui_ware_lock.jpg", IMAGE_STORAGE_BTN_LOCK);

	LoadBitmap("Common\\Interface\\ui_item_money.jpg", IMAGE_STORAGE_MONEY);
}
...
void CNewUIStorageInventory::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_STORAGE_BTN_ZEN);
	DeleteBitmap(IMAGE_STORAGE_BTN_ZEN_CLICK);
	DeleteBitmap(IMAGE_STORAGE_TOP);
	DeleteBitmap(IMAGE_STORAGE_LOW);
}
...
{
	if (SEASON3B::CheckMouseIn(m_Pos.x + 38, m_Pos.y + 390, 24, 24) == true && MouseLButtonPush)
	{
		...
		SEASON3B::CreateMessageBox(MSGBOX_LAYOUT_CLASS(SEASON3B::CZenReceiptMsgBoxLayout));

		PlayBuffer(SOUND_CLICK01);

		return true;
	}
	else if (SEASON3B::CheckMouseIn(m_Pos.x + 78, m_Pos.y + 390, 24, 24) == true && MouseLButtonPush)
	{
		MouseLButtonPush = false;
	...
	else if (SEASON3B::CheckMouseIn(m_Pos.x + 118, m_Pos.y + 390, 24, 24) == true && MouseLButtonPush)
	{
		MouseLButtonPush = false;
