...
bool SEASON3B::CNewUISystem::LoadMainSceneInterface()
{
	...
	if(m_pNewChatInputBox->Create(m_pNewUIMng, m_pNewChatLogWindow, 179, 385) == false)
	{
		return false;
	}
	...
	if (m_pNewItemEnduranceInfo->Create(m_pNewUIMng, 640, 0) == false)
	{
		return false;
	}
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	if(m_pNewUIMng) 
	{
		...
		else if(dwKey == SEASON3B::INTERFACE_MYSHOP_INVENTORY)
		{
			...
			{
				g_pMyShopInventory->OpenButtonState(0);
			}
			else
			{
				g_pMyShopInventory->OpenButtonState(1);
			}
		}
