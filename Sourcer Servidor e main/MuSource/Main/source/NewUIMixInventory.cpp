...
{
	LoadBitmap("Common\\Interface\\ui_item_backtop.jpg", IMAGE_MIXINVENTORY_TOP);
	LoadBitmap("Common\\Interface\\ui_item_backlow.jpg", IMAGE_MIXINVENTORY_LOW);

	LoadBitmap("Common\\Interface\\ui_mix_button.jpg", IMAGE_MIXINVENTORY_MIXBTN);
	LoadBitmap("Common\\Interface\\ui_mix_button_click.jpg", IMAGE_MIXINVENTORY_MIXBTN_CLICK);
}

void CNewUIMixInventory::UnloadImages()
{
	DeleteBitmap(IMAGE_MIXINVENTORY_TOP);
	DeleteBitmap(IMAGE_MIXINVENTORY_LOW);

	DeleteBitmap(IMAGE_MIXINVENTORY_MIXBTN);
	DeleteBitmap(IMAGE_MIXINVENTORY_MIXBTN_CLICK);
}

void CNewUIMixInventory::RenderFrame()
{
	RenderBitmap(IMAGE_MIXINVENTORY_TOP, m_Pos.x, m_Pos.y, 190.0, 256.0, 0, 0, 0.7421875, 1.0, 1, 1, 0.0);
	RenderBitmap(IMAGE_MIXINVENTORY_LOW, m_Pos.x, m_Pos.y + 256.0, 190.0, 177.0, 0, 0, 0.7421875, 0.69140625, 1, 1, 0.0);
	...
	g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y, szText, 160, 0, RT3_SORT_CENTER);
	...
	case SEASON3A::MIXTYPE_ELPIS:
	case SEASON3A::MIXTYPE_TRAINER:
		if (g_MixRecipeMgr.IsReadyToMix() && g_MixRecipeMgr.GetPlusChaosRate() > 0 && g_MixRecipeMgr.GetCurRecipe()->m_bMixOption == 'F')
		{
			g_pRenderText->SetTextColor(255, 255, 48, 255);
			g_pRenderText->SetBgColor(40, 40, 40, 128);
	...
	{
	case SEASON3A::MIXTYPE_TRAINER:
		sprintf(szBuffer, GlobalText[1212]);
		break;
	case SEASON3A::MIXTYPE_OSBOURNE:
		sprintf(szBuffer, GlobalText[2061]);
		break;
	case SEASON3A::MIXTYPE_JERRIDON:
		sprintf(szBuffer, GlobalText[2062]);
		break;
	case SEASON3A::MIXTYPE_ELPIS:
		sprintf(szBuffer, GlobalText[2063]);
		break;
	default:
		sprintf(szBuffer, GlobalText[591]);
		break;
	}

	RenderBitmap(this->iClickState == FALSE ? IMAGE_MIXINVENTORY_MIXBTN : IMAGE_MIXINVENTORY_MIXBTN_CLICK, (m_Pos.x + INVENTORY_WIDTH * 0.5f - 22.f), (m_Pos.y + 380), 44.0, 32.0, 0, 0, (44.0 / 64.0), (32.0 / 32.0), 1, 1, 0.0);

	if (SEASON3B::CheckMouseIn((m_Pos.x + INVENTORY_WIDTH * 0.5f - 22.f), (m_Pos.y + 380), 44.0, 32.0) == true)
	{
		RenderTipText((m_Pos.x + INVENTORY_WIDTH * 0.5f - 22.f), (m_Pos.y + 370), szBuffer);
	}
}
...
bool CNewUIMixInventory::BtnProcess()
{
	...
	if (SEASON3B::CheckMouseIn((m_Pos.x + INVENTORY_WIDTH * 0.5f - 22.f), (m_Pos.y + 380), 44.0, 32.0) == true && MouseLButtonPush)
	{
...
bool CNewUIMixInventory::Mix()
{
	...
	if (g_MixRecipeMgr.GetCurRecipe()->m_iWidth != -1 && g_pMyInventory->FindEmptySlot(g_MixRecipeMgr.GetCurRecipe()->m_iWidth, g_MixRecipeMgr.GetCurRecipe()->m_iHeight) == -1)
	{
