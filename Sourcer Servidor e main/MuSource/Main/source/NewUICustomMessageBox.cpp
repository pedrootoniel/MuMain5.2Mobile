...
bool SEASON3B::CNewUITextInputMsgBox::Render()
{
	...
	x = GetPos().x; y = GetPos().y + 2.f;

	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_LINE, x, y, MSGBOX_LINE_WIDTH, MSGBOX_LINE_HEIGHT, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);

	y += MSGBOX_LINE_HEIGHT; width = MSGBOX_WIDTH; height = MSGBOX_BOTTOM_HEIGHT;

	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BACK, x, y, width, height, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);

	y += MSGBOX_BOTTOM_HEIGHT;
	...
	width = MSGBOX_WIDTH; height = MSGBOX_LINE_HEIGHT;
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_LINE, x, y, width, height, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);
...
void SEASON3B::CNewUIKeyPadMsgBox::RenderFrame()
{
	...
	x = GetPos().x; y = GetPos().y - 5.0f, width = GetSize().cx - MSGBOX_LINE_HEIGHT; height = GetSize().cy - MSGBOX_BACK_BLANK_HEIGHT;
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_LINE, x, y, MSGBOX_LINE_WIDTH, MSGBOX_LINE_HEIGHT, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);
...
void SEASON3B::CUseFruitCheckMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
void SEASON3B::CGemIntegrationMsgBox::SetButtonInfo()
{
	...
	x = GetPos().x + msgboxhalfwidth - btnhalf + 5;
	y += 50;

	width = 49;
	height = 16;
	m_BtnCancel.SetInfo(ButtonIndex::Button_Small, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);

	m_BtnCancel.SetText(GlobalText[1002]);
}

void SEASON3B::CGemIntegrationMsgBox::RenderFrame() // -> Lahap
{
	float x, y, width, height;

	x = GetPos().x; y = GetPos().y - 5.0f;
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_LINE, x, y, MSGBOX_LINE_WIDTH, MSGBOX_LINE_HEIGHT, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);
...
void SEASON3B::CGemIntegrationUnityMsgBox::ResetWndSize( int _nType )
{
	...
	SetSize(GetSize().cx, height);
	m_BtnCancel.SetPos(m_BtnCancel.GetPosX(), GetPos().y + GetSize().cy - 10 );
}
...
void SEASON3B::CGemIntegrationUnityMsgBox::SetButtonInfo()
{
	...
	int	  nNum = 0;
	int	  nBtnIndex[COMGEM::eGEMTYPE_END] = { 1806, 1807, 3200, 3201, 3202, 2081, 3203, 3204, 3205, 3206 };
	...
	{
		cButton.SetInfo(ButtonIndex::Button_Small, x + 20.0f + (i % 2) * (20 + width), y + (height + 5.0f) * int(i / 2), width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	...
	{
		cButton.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY, x + 15.0f, y + (height + 22.0f) * k, MSGBOX_BTN_EMPTY_WIDTH + 20, height + 12, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	...
	y += (height) * (int)COMGEM::eCOMTYPE_END;
...
void SEASON3B::CGemIntegrationUnityMsgBox::RenderFrame() // -> Lahap
{
	float x, y, width, height;
...
void SEASON3B::CGemIntegrationDisjointMsgBox::SetButtonInfo()
{
	...
	m_BtnDisjoint.SetInfo(ButtonIndex::Button_Small, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	m_BtnDisjoint.SetText(GlobalText[188]);
	m_BtnDisjoint.SetEnable(false);
}

void SEASON3B::CGemIntegrationDisjointMsgBox::RenderFrame() // -> Lahap
{
	float x, y, width, height;
...
bool SEASON3B::CDevilSquareRankMsgBox::Create(float fPriority)
{
	...
	x = GetPos().x + (GetSize().cx / 2) - (MSGBOX_BTN_WIDTH / 2);
	y = GetPos().y + GetSize().cy - (MSGBOX_BTN_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK) + 20;
	width = MSGBOX_BTN_WIDTH;
	height = MSGBOX_BTN_HEIGHT;
	m_BtnOk.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_OK, x, y, width, height);
...
void SEASON3B::CDevilSquareRankMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
bool SEASON3B::CChaosCastleResultMsgBox::Create(float fPriority)
{
	...
	x = GetPos().x + (GetSize().cx / 2) - (MSGBOX_BTN_WIDTH / 2);
	y = GetPos().y + GetSize().cy - (MSGBOX_BTN_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK) + 10;
	width = MSGBOX_BTN_WIDTH;
	height = MSGBOX_BTN_HEIGHT;
...
void SEASON3B::CChaosCastleResultMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
void SEASON3B::CDialogMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
void SEASON3B::CProgressMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
void SEASON3B::CCursedTempleProgressMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
void SEASON3B::CDuelMsgBox::RenderFrame() // -> Duel
{
	float x, y, width, height;
...
void CCherryBlossomMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
void SEASON3B::CElpisMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
void SEASON3B::CResetCharacterPointMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
...
void SEASON3B::CGuild_ToPerson_Position::SetButtonInfo()
{
	...
	m_BtnBlessing.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnBlessing.SetText(GlobalText[1311]);

	/* */

	y += 30;
	m_BtnSoul.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	...
	y += 75;
	m_BtnOk.SetInfo((BITMAP_INTERFACE_EX + 9), x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	...
	m_BtnCancel.SetInfo((BITMAP_INTERFACE_EX + 9), x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	m_BtnCancel.SetText(GlobalText[1002]);
}
...
void SEASON3B::CGuild_ToPerson_Position::RenderTexts()
{
	...
	x = GetPos().x; y = (GetPos().y + (MSGBOX_TEXT_TOP_BLANK / 2)) + 70;
...
void SEASON3B::CDelgardoMainMenuMsgBox::RenderFrame() // Refazer
{
	float x, y, width, height;
