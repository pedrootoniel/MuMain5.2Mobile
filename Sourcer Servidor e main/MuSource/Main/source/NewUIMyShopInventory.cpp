...
bool SEASON3B::CNewUIMyShopInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_EditBox->SetPosition(m_Pos.x + 60, m_Pos.y + 65);
	...
	m_EditBox->SetText("Digite o nome da loja");
...
{
	LoadBitmap("Common\\Interface\\ui_item_backtop.jpg", IMAGE_MYSHOPINVENTORY_TOP);
	LoadBitmap("Common\\Interface\\ui_item_backlow.jpg", IMAGE_MYSHOPINVENTORY_LOW);
	LoadBitmap("Common\\Interface\\ui_item_money.jpg", IMAGE_MYSHOPINVENTORY_EDIT);
	LoadBitmap("Common\\Interface\\ui_menu_pshop_btn_box.jpg", IMAGE_MYSHOPINVENTORY_BOX);
}

void SEASON3B::CNewUIMyShopInventory::UnloadImages()
{
	DeleteBitmap(IMAGE_MYSHOPINVENTORY_BOX);
	DeleteBitmap(IMAGE_MYSHOPINVENTORY_EDIT);	
	DeleteBitmap(IMAGE_MYSHOPINVENTORY_TOP);
	DeleteBitmap(IMAGE_MYSHOPINVENTORY_LOW);
}
...
void SEASON3B::CNewUIMyShopInventory::OpenButtonState(int State)
{
	if (State == 0)
	{
...
bool SEASON3B::CNewUIMyShopInventory::UpdateMouseEvent()
{
	...
	if (CheckMouseIn(m_Pos.x, m_Pos.y, INVENTORY_WIDTH, INVENTORY_HEIGHT))
	{
		...
		if (SEASON3B::IsRelease(VK_LBUTTON) && CheckMouseIn(m_EditBox->GetPosition_x(), m_EditBox->GetPosition_y(), m_EditBox->GetWidth(), m_EditBox->GetHeight()))
		{
			ChangeEditBox(UISTATE_NORMAL);
		}

		if (SEASON3B::IsRelease(VK_LBUTTON) && CheckMouseIn(m_EditBox->GetPosition_x(), m_EditBox->GetPosition_y(), m_EditBox->GetWidth(), m_EditBox->GetHeight()) == false)
		{
			SetFocus(g_hWnd);
		}
	}
	...
	if (this->m_ButtonState == true && CheckMouseIn(m_Pos.x + 30, m_Pos.y + 391, 56, 24) == true && MouseLButtonPush)
	{
	...
	if (CheckMouseIn(m_Pos.x + 105, m_Pos.y + 391, 56, 24) == true && MouseLButtonPush)
	{
		MouseLButtonPush = false;
...
{
	RenderBitmap(IMAGE_MYSHOPINVENTORY_TOP, m_Pos.x, m_Pos.y, 190.0, 256.0, 0, 0, 0.7421875, 1.0, 1, 1, 0.0);
	RenderBitmap(IMAGE_MYSHOPINVENTORY_LOW, m_Pos.x, m_Pos.y + 256.0, 190.0, 177.0, 0, 0, 0.7421875, 0.69140625, 1, 1, 0.0);

	RenderBitmap(IMAGE_MYSHOPINVENTORY_EDIT, m_Pos.x + 20, m_Pos.y + 60, 150.0, 24.0, 0, 0, 0.8828125, 0.5625, 1, 1, 0.0);

	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->SetBgColor(30, 30, 30, 255);
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->RenderText(m_Pos.x + 36, m_Pos.y + 12, GlobalText[1102], 120, 0, RT3_SORT_CENTER);
	...
	if (m_EnablePersonalShop == false)
	{
		g_pRenderText->SetBgColor(139, 0, 0, 255);
		g_pRenderText->RenderText(m_Pos.x + 20, m_Pos.y + 46, "Preparing", 150, 0, RT3_SORT_CENTER);
	}
	else
	{
		g_pRenderText->SetBgColor(76, 187, 23, 255);
		g_pRenderText->RenderText(m_Pos.x + 20, m_Pos.y + 46, "Open", 150, 0, RT3_SORT_CENTER);
	}
	...
	if (SEASON3B::CheckMouseIn(m_Pos.x + 30, m_Pos.y + 391, 56, 24) == true)
	{
	...
	RenderBitmap(IMAGE_MYSHOPINVENTORY_BOX, m_Pos.x + 30, m_Pos.y + 391, 56.0, 24.0, 0, 0, 0.875, 0.75, 1, 1, 0.0);

	glColor3f(1.0, 1.0, 1.0);

	g_pRenderText->RenderText(m_Pos.x + 50, m_Pos.y + 398, "Open", 60, 0, RT3_SORT_LEFT);

	/* */

	if (SEASON3B::CheckMouseIn(m_Pos.x + 105, m_Pos.y + 391, 56, 24) == true)
	{
		glColor3f(1.0, 1.0, 1.0);
	}
	else
	...
	RenderBitmap(IMAGE_MYSHOPINVENTORY_BOX, m_Pos.x + 105, m_Pos.y + 391, 56.0, 24.0, 0, 0, 0.875, 0.75, 1, 1, 0.0);

	glColor3f(1.0, 1.0, 1.0);

	g_pRenderText->RenderText(m_Pos.x + 125, m_Pos.y + 398, "Close", 60, 0, RT3_SORT_LEFT);
}
