...
bool CSItemOption::OpenItemSetScript()
{
	...
	strFileName = "Data\\Local\\" + g_strSelectedML + "\\ItemSetOption" + "_" + g_strSelectedML + ".bmd";
...
void    CSItemOption::getAllAddState(WORD* Strength, WORD* Dexterity, WORD* Energy, WORD* Vitality, WORD* Charisma)
{
	for (int i = EQUIPMENT_WEAPON_RIGHT; i < MAX_EQUIPMENT; ++i)
	{
		...
		if (item->Durability <= 0 || (IsRequireEquipItem(item) == false))
		{
			continue;
		}
...
int CSItemOption::RenderSetOptionListInItem(const ITEM* ip, int TextNum, bool bIsEquippedItem)
{
	...
	for (int i = 0; i <= MAX_SETITEM_OPTIONS; ++i)
	{
		...
		if (option1 != 255)
		{
			...
			if (m_bySetOptionList[count1][0] == option1 && byLimitOptionNum != 255 && iLimitOptionCount > i && byLimitOptionNum != 0 && bIsEquippedItem == false)
			{
				TextListColor[TNum] = TEXT_COLOR_BLUE;
				count1++;
			}
		...
		if (option2 != 255)
		{
			...
			if (m_bySetOptionList[count1][0] == option2 && byLimitOptionNum != 255 && iLimitOptionCount > i && byLimitOptionNum != 0)
			{
				TextListColor[TNum] = TEXT_COLOR_BLUE;
				count1++;
			}
