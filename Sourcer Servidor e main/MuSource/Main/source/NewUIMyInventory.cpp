...
bool SEASON3B::CNewUIMyInventory::IsEquipable(int iIndex, ITEM* pItem)
{
	...
	if (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_ELF)
	{
		...
		if (iIndex == EQUIPMENT_WEAPON_RIGHT && l->Type != ITEM_BOW + 7 && (l->Type >= ITEM_BOW && l->Type < ITEM_BOW + MAX_ITEM_INDEX))
		{
...
bool SEASON3B::CNewUIMyInventory::UpdateKeyEvent()
{
	if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_INVENTORY))
	{
		...
		else if (SEASON3B::IsPress('T') == true)
		{
			if (m_bRepairEnableLevel == true && g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_NPCSHOP) == false	&& g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MIXINVENTORY) == false)
			{
				ToggleRepairMode();
...
void SEASON3B::CNewUIMyInventory::Render3D()
{
	for (int i = 0; i < MAX_EQUIPMENT_INDEX; i++)
	{
		...
		if (pEquippedItem->Type >= 0)
		{
			...
			{
				y = m_EquipmentSlots[i].y - 8.f;
			}
			else
			{
				y = m_EquipmentSlots[i].y + 2.f;
			}

			if (i == EQUIPMENT_AMULET)
			{
				y = m_EquipmentSlots[i].y - 2.f;
			}
...
void SEASON3B::CNewUIMyInventory::SetEquipmentSlotInfo()
{
	...
	m_EquipmentSlots[EQUIPMENT_WEAPON_LEFT].dwBgImage = IMAGE_INVENTORY_ITEM_RIGHT;
	...
	m_EquipmentSlots[EQUIPMENT_WEAPON_RIGHT].dwBgImage = IMAGE_INVENTORY_ITEM_LEFT;
...
{
	LoadBitmap("Common\\Interface\\ui_item_backtop.jpg", IMAGE_INVENTORY_BACK_TOP);
	LoadBitmap("Common\\Interface\\ui_item_backlow.jpg", IMAGE_INVENTORY_BACK_LOW);
	LoadBitmap("Common\\Interface\\ui_menu_setitem.jpg", IMAGE_INVENTORY_SET_ITEM);
	
	LoadBitmap("Common\\Interface\\ui_item_slot_boots.jpg", IMAGE_INVENTORY_ITEM_BOOT);
	LoadBitmap("Common\\Interface\\ui_item_slot_helm.jpg", IMAGE_INVENTORY_ITEM_HELM);
	LoadBitmap("Common\\Interface\\ui_item_slot_pet.jpg", IMAGE_INVENTORY_ITEM_FAIRY);
	LoadBitmap("Common\\Interface\\ui_item_slot_wing.jpg", IMAGE_INVENTORY_ITEM_WING);
	LoadBitmap("Common\\Interface\\ui_item_slot_weapon_left.jpg", IMAGE_INVENTORY_ITEM_LEFT);
	LoadBitmap("Common\\Interface\\ui_item_slot_weapon_right.jpg", IMAGE_INVENTORY_ITEM_RIGHT);
	LoadBitmap("Common\\Interface\\ui_item_slot_armor.jpg", IMAGE_INVENTORY_ITEM_ARMOR);
	LoadBitmap("Common\\Interface\\ui_item_slot_gloves.jpg", IMAGE_INVENTORY_ITEM_GLOVES);
	LoadBitmap("Common\\Interface\\ui_item_slot_pants.jpg", IMAGE_INVENTORY_ITEM_PANTS);
	LoadBitmap("Common\\Interface\\ui_item_slot_ring.jpg", IMAGE_INVENTORY_ITEM_RING);
	LoadBitmap("Common\\Interface\\ui_item_slot_pendant.jpg", IMAGE_INVENTORY_ITEM_NECKLACE);

	LoadBitmap("Common\\Interface\\ui_item_money.jpg", IMAGE_INVENTORY_MONEY);

	LoadBitmap("Common\\Interface\\ui_menu_exit.jpg", IMAGE_INVENTORY_EXIT_BTN);

	LoadBitmap("Common\\Interface\\ui_menu_repair.jpg", IMAGE_INVENTORY_REPAIR_BTN);
	LoadBitmap("Common\\Interface\\ui_menu_repair_click.jpg", IMAGE_INVENTORY_REPAIR_CLK);

	LoadBitmap("Common\\Interface\\ui_menu_pshop.jpg", IMAGE_INVENTORY_MYSHOP_OPEN_BTN);
	LoadBitmap("Common\\Interface\\ui_menu_pshop_click.jpg", IMAGE_INVENTORY_MYSHOP_CLOSE_BTN);
}
...
void SEASON3B::CNewUIMyInventory::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_INVENTORY_BACK_LOW);
	DeleteBitmap(IMAGE_INVENTORY_SET_ITEM);
}

void SEASON3B::CNewUIMyInventory::RenderFrame()
{
	RenderBitmap(IMAGE_INVENTORY_BACK_TOP, m_Pos.x, m_Pos.y, 190.0, 256.0, 0, 0, 0.7421875, 1.0, 1, 1, 0.0);
	RenderBitmap(IMAGE_INVENTORY_BACK_LOW, m_Pos.x, m_Pos.y + 256.0, 190.0, 177.0, 0, 0, 0.7421875, 0.69140625, 1, 1, 0.0);
...
void SEASON3B::CNewUIMyInventory::RenderEquippedItem()
{
	for (int i = 0; i < MAX_EQUIPMENT_INDEX; i++)
	{
		...
		if (pEquipmentItemSlot->Type != -1)
		{
			...
			{
				if (pEquipmentItemSlot->Type == ITEM_HELPER + 20 && iLevel == 1	|| iLevel == 2)
				{
					continue;
				}
			}
			...
			RenderColor(m_EquipmentSlots[i].x + 1, m_EquipmentSlots[i].y, m_EquipmentSlots[i].width - 1, m_EquipmentSlots[i].height - 1);
	...
	if (CNewUIInventoryCtrl::GetPickedItem() && m_iPointedSlot != -1)
	{
		...
		if (pItemObj && (pEquipmentItemSlot->Type != -1 || false == IsEquipable(m_iPointedSlot, pItemObj)))
		{
			...
			RenderColor(m_EquipmentSlots[m_iPointedSlot].x + 1, m_EquipmentSlots[m_iPointedSlot].y,	m_EquipmentSlots[m_iPointedSlot].width - 1, m_EquipmentSlots[m_iPointedSlot].height - 1);
...
void SEASON3B::CNewUIMyInventory::RenderButtons()
{
	...
	{
		sprintf(szBuffer, "Close (V)");

		RenderTipText(m_Pos.x + 20, m_Pos.y + 380, szBuffer);
	}

	RenderBitmap(IMAGE_INVENTORY_EXIT_BTN, m_Pos.x + 23, m_Pos.y + 395, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);
		...
		&& g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_STORAGE) == false)
	{
		if (m_bRepairEnableLevel == true)
		{
			...
			{
				sprintf(szBuffer, "Repair (T)");

				RenderTipText(m_Pos.x + 50, m_Pos.y + 380, szBuffer);
			}

			if (GetRepairMode() == REPAIR_MODE_OFF)
			{
				RenderBitmap(IMAGE_INVENTORY_REPAIR_BTN, m_Pos.x + 53, m_Pos.y + 395, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);
			}
			else
			{
				RenderBitmap(IMAGE_INVENTORY_REPAIR_CLK, m_Pos.x + 53, m_Pos.y + 395, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);
			}
		}
		...
		if (m_bMyShopOpen == true)
		{
			...
			{
				sprintf(szBuffer, "Personal Store (S)");
			...
			{
				RenderBitmap(IMAGE_INVENTORY_MYSHOP_CLOSE_BTN, m_Pos.x + 83, m_Pos.y + 395, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);
			}
			else
			{
				RenderBitmap(IMAGE_INVENTORY_MYSHOP_OPEN_BTN, m_Pos.x + 83, m_Pos.y + 395, 24.0, 24.0, 0, 0, 0.75, 0.75, 1, 1, 0.0);
			}
		}
	}
...
void SEASON3B::CNewUIMyInventory::RenderInventoryDetails()
{
	...
	g_pRenderText->RenderText(m_Pos.x + 36, m_Pos.y + 12, GlobalText[223], 120, 0, RT3_SORT_CENTER);

	RenderBitmap(IMAGE_INVENTORY_MONEY, m_Pos.x + 45, m_Pos.y + 366, 110.0, 18.0, 0, 0, 0.859375, 0.5625, 1, 1, 0.0);
	...
	if (rPCRoomPtSys.IsPCRoomPointShopMode())
	{
		...
		g_pRenderText->SetTextColor((255 << 24) + (150 << 16) + (220 << 8) + (255));
		g_pRenderText->RenderText(m_Pos.x + 32, m_Pos.y + 370, GlobalText[2326], 0, 0, RT3_WRITE_RIGHT_TO_LEFT);
	}
	else
	{
		...
		g_pRenderText->SetTextColor((255 << 24) + (150 << 16) + (220 << 8) + (255));
		g_pRenderText->RenderText(m_Pos.x + 32, m_Pos.y + 370, GlobalText[100], 0, 0, RT3_WRITE_RIGHT_TO_LEFT);
	}
...
bool SEASON3B::CNewUIMyInventory::BtnProcess()
{
	...
	if (m_bRepairEnableLevel == true && SEASON3B::CheckMouseIn(m_Pos.x + 56, m_Pos.y + 395, 24, 24) == true && MouseLButtonPush)
	{
		MouseLButtonPush = false;
	...
	if (m_bMyShopOpen == true && SEASON3B::CheckMouseIn(m_Pos.x + 83, m_Pos.y + 395, 24, 24) == true && MouseLButtonPush)
	{
		MouseLButtonPush = false;
