...
void CreateBord(int X, int Y)
{
	DisableAlphaBlend();

	glColor4ub(146, 134, 121, 255);

	RenderColor(X, Y, 1.0, 24.0);

	RenderColor((X + 74), Y, 1.0, 24.0);
...
bool CNewUIPartyListWindow::Render()
{
	...
	for (int i = 0; i < PartyNumber; i++)
	{
		...
		RenderColor(float(m_Pos.x), float(m_Pos.y + 2 + iVal), PARTY_LIST_WINDOW_WIDTH, PARTY_LIST_WINDOW_HEIGHT);
		EnableAlphaTest();

		CreateBord(float(m_Pos.x), float(m_Pos.y + 2 + iVal));

		if (Party[i].index == -1)
		{
			glColor4f(0.3f, 0.f, 0.f, 0.5f);
			RenderColor(m_Pos.x, m_Pos.y + 2 + iVal, PARTY_LIST_WINDOW_WIDTH, PARTY_LIST_WINDOW_HEIGHT);
			EnableAlphaTest();
		}
		else
		{
		...
		float PartyChangePosY = 0;

		if (PartyNumber == 1) { PartyChangePosY += 24.f; }
		if (PartyNumber == 2) { PartyChangePosY += 48.f; }
		if (PartyNumber == 3) { PartyChangePosY += 72.f; }
		if (PartyNumber == 4) { PartyChangePosY += 96.f; }
		if (PartyNumber == 5) { PartyChangePosY += 120.f; }

		RenderBitmap(IMAGE_PARTY_LIST_LINE, m_Pos.x - 2, m_Pos.y - 1.0, 80.0, 3.0, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);
		RenderBitmap(IMAGE_PARTY_LIST_LINE, m_Pos.x - 2, m_Pos.y + PartyChangePosY + 2, 80.0, 3.0, 0, 0, 0.83203125, 0.625, 1, 1, 0.0);
		...
		RenderImage(IMAGE_PARTY_LIST_HPBAR, m_Pos.x + 3, m_Pos.y + 16 + iVal, fLife, 3);
	}
