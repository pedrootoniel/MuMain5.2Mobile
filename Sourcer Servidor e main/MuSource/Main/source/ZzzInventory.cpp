...
void RenderItemInfo(int sx,int sy,ITEM *ip,bool Sell, int Inventype, bool bItemTextListBoxUse)
{
	...
	if(	(Inventype == SEASON3B::TOOLTIP_TYPE_MY_SHOP || Inventype == SEASON3B::TOOLTIP_TYPE_PURCHASE_SHOP)	&& !IsPersonalShopBan(ip) )
	{
		{
			int price = 0;
    ...
    else if ( ip->Type==ITEM_HELPER+29 )
    {
        ...
        for ( int i=0; i<6; i++ )
        {
            ...
            if ( (HeroLevel>=g_iChaosCastleLevel[startIndex+i][0] && HeroLevel<=g_iChaosCastleLevel[startIndex+i][1]) && gCharacterManager.IsThirdClass(Hero->Class) == false)
            {
                TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW; 
            }
            else
		...
		if(gCharacterManager.IsThirdClass(Hero->Class) == true)
		{
			TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW; 
		}
		else
	...
	if( ip->bPeriodItem == true )
	{
		...
		else
		{
			...
			sprintf(TextList[TextNum], "%d-%02d-%02d  %02d:%02d", ExpireTime->tm_year+1900, ExpireTime->tm_mon+1, ExpireTime->tm_mday, ExpireTime->tm_hour, ExpireTime->tm_min);
			TextListColor[TextNum] = TEXT_COLOR_BLUE; 
		}
