...
bool SEASON3B::CNewUIOptionWindow::UpdateMouseEvent()
{
	...
	if(SEASON3B::IsPress(VK_LBUTTON) && CheckMouseIn(m_Pos.x + 60, m_Pos.y+43, 15, 15))
	{
		m_bAutoAttack = !m_bAutoAttack;
	}

	if(SEASON3B::IsPress(VK_LBUTTON) && CheckMouseIn(m_Pos.x + 60, m_Pos.y+65, 15, 15))
	{
		m_bWhisperSound = !m_bWhisperSound;
	}

	if(SEASON3B::IsPress(VK_LBUTTON) && CheckMouseIn(m_Pos.x + 60, m_Pos.y+127, 15, 15))
	{
		m_bSlideHelp = !m_bSlideHelp;
	}
	
	if (SEASON3B::IsPress(VK_LBUTTON) && CheckMouseIn(m_Pos.x + 240, m_Pos.y + 43, 15, 15))
	{
		/*m_bMusic = !m_bMusic;*/
	}

	if (CheckMouseIn((m_Pos.x - 60 - 8), m_Pos.y + 104, 124 + 8, 16))
	{
	...
	if (CheckMouseIn((m_Pos.x - 60), m_Pos.y + 168, 141, 29))
	{
		if (SEASON3B::IsRepeat(VK_LBUTTON))
		{
			int x = MouseX - (m_Pos.x - 60);
	...
	if (CheckMouseIn((m_Pos.x - 100), m_Pos.y, 400, 220) == true)
	{
		return false;
	}
...
void SEASON3B::CNewUIOptionWindow::LoadImages()
{
	...
	LoadBitmap("Common\\Interface\\ui_msgbox_back.jpg", IMAGE_OPTION_FRAME_BACK);
...
void SEASON3B::CNewUIOptionWindow::RenderFrame()
{
	...
	RenderColor(x, y, 400, 200);

	EndRenderColor();

	gInterfaceManager.RenderBoxGoldRect(x, y, 400, 200);
}
...
{	
	...
	g_pRenderText->RenderText(m_Pos.x - 60, m_Pos.y+48, GlobalText[386]);
	g_pRenderText->RenderText(m_Pos.x - 60, m_Pos.y+70, GlobalText[387]);
	g_pRenderText->RenderText(m_Pos.x - 60, m_Pos.y+92, GlobalText[389]);
	g_pRenderText->RenderText(m_Pos.x - 60, m_Pos.y+132, GlobalText[919]);
	g_pRenderText->RenderText(m_Pos.x - 60, m_Pos.y+154, GlobalText[1840]);

	g_pRenderText->RenderText(m_Pos.x + 140, m_Pos.y + 48, "Music Option");
}
...
void SEASON3B::CNewUIOptionWindow::RenderButtons()
{
	...
	if (m_bAutoAttack)
	{
		RenderImage(IMAGE_OPTION_BTN_CHECK, m_Pos.x + 60, m_Pos.y + 43, 15, 15, 0, 0);
	}
	else
	{
		RenderImage(IMAGE_OPTION_BTN_CHECK, m_Pos.x + 60, m_Pos.y + 43, 15, 15, 0, 15.f);
	}

	if (m_bWhisperSound)
	{
		RenderImage(IMAGE_OPTION_BTN_CHECK, m_Pos.x + 60, m_Pos.y + 65, 15, 15, 0, 0);
	}
	else
	{
		RenderImage(IMAGE_OPTION_BTN_CHECK, m_Pos.x + 60, m_Pos.y + 65, 15, 15, 0, 15.f);
	}

	if (m_bSlideHelp)
	{
		RenderImage(IMAGE_OPTION_BTN_CHECK, m_Pos.x + 60, m_Pos.y + 127, 15, 15, 0, 0);
	}
	else
	{
		RenderImage(IMAGE_OPTION_BTN_CHECK, m_Pos.x + 60, m_Pos.y + 127, 15, 15, 0, 15.f);
	}

	RenderImage(IMAGE_OPTION_VOLUME_BACK, m_Pos.x - 60, m_Pos.y + 104, 124.f, 16.f);

	if (m_iVolumeLevel > 0)
	{
		RenderImage(IMAGE_OPTION_VOLUME_COLOR, m_Pos.x - 60, m_Pos.y + 104, 124.f * 0.1f * (m_iVolumeLevel), 16.f);
	}

	RenderImage(IMAGE_OPTION_EFFECT_BACK, m_Pos.x - 60, m_Pos.y + 168, 141.f, 29.f);

	if (m_iRenderLevel >= 0)
	{
		RenderImage(IMAGE_OPTION_EFFECT_COLOR, m_Pos.x - 60, m_Pos.y + 168, 141.f * 0.2f * (m_iRenderLevel + 1), 29.f);
	}
	...
	if (m_bAutoAttack)
	{
		RenderImage(IMAGE_OPTION_BTN_CHECK, m_Pos.x + 240, m_Pos.y + 43, 15, 15, 0, 0);
	}
	else
	{
		RenderImage(IMAGE_OPTION_BTN_CHECK, m_Pos.x + 240, m_Pos.y + 43, 15, 15, 0, 15.f);
	}
}
