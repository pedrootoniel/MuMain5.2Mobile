...
{
	LoadBitmap("Common\\Interface\\ui_item_box.jpg", IMAGE_ITEM_SQUARE);
	LoadBitmap("Common\\Interface\\ui_inventory_box.jpg", IMAGE_INVENTORY_BOX);
}

void SEASON3B::CNewUIInventoryCtrl::UnloadImages()
{
	DeleteBitmap(IMAGE_ITEM_SQUARE);
	DeleteBitmap(IMAGE_INVENTORY_BOX);
}
...
void SEASON3B::CNewUIInventoryCtrl::Render()
{
	...
	for (y = 0; y < m_nRow; y++)
	{
		for (x = 0; x < m_nColumn; x++)
		{
			...
			RenderBitmap(IMAGE_ITEM_SQUARE, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21, 21, 0, 0, 1.0, 1.0, 1, 1, 0.0);
			...
			if (m_pdwItemCheckBox[iCurSquareIndex] > 1)
			{
				...
				if (pItem)
				{
					RenderBitmap(IMAGE_INVENTORY_BOX, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21, 21, 0, 0, 1.0, 1.0, 1, 1, 0.0);

					switch (pItem->byColorState)
					{
					case ITEM_COLOR_NORMAL:
						break;
					case ITEM_COLOR_DURABILITY_50:
						glColor4f(1.0, 1.0, 0.0, 0.30000001);
						break;
					case ITEM_COLOR_DURABILITY_70:
						glColor4f(1.0, 0.66000003, 0.0, 0.30000001);
						break;
					case ITEM_COLOR_DURABILITY_80:
						glColor4f(0.89000001, 0.10900001, 0.10900001, 0.30000001);
						break;
					...
					if (pItem->byColorState >= 1 && pItem->byColorState <= 5)
					{
						glColor3f(1.0, 1.0, 1.0);

						RenderColor(m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21, 21);
									...
									if (pTargetItem)
									{
										...
										else if (pPickItem->Type == ITEM_POTION + 42)
										{
											if (pTargetItem->Jewel_Of_Harmony_Option == 0)
											{
												...
												if ((strengthitem != SI_None) && ((pTargetItem->ExtOption % 0x04) != EXT_A_SET_OPTION && (pTargetItem->ExtOption % 0x04) != EXT_B_SET_OPTION))
												{
													bSuccess = true;
												}
											}
