...
void CMonsterSkillElementInfo::ApplyElementSummon(int aIndex,int bIndex) // OK
{
	...
	gMonster->InitHitDamage(&gObj[bIndex]);
...
void CMonsterSkillElementInfo::ApplyElementNormalAttack(int aIndex,int bIndex) // OK
{
	...
	{
		if(gObj[bIndex].Inventory[INVENTORY_SLOT_AMULET].m_Index != GET_ITEM(this->m_IncAndDecType,this->m_IncAndDecValue) && gObj[bIndex].Inventory[INVENTORY_SLOT_RING1].m_Index != GET_ITEM(this->m_IncAndDecType,this->m_IncAndDecValue) && gObj[bIndex].Inventory[INVENTORY_SLOT_RING2].m_Index != GET_ITEM(this->m_IncAndDecType,this->m_IncAndDecValue))
		{
			gObj[bIndex].Life = 0;
			gObjectManager->CharacterLifeCheck(&gObj[aIndex],&gObj[bIndex],(int)(gObj[bIndex].MaxLife+gObj[bIndex].AddLife),0,0,0,0,0,0);
			return;
		}
	}
