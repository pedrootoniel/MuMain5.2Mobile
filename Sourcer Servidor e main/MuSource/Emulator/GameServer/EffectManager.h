...
class CEffectManager
{
	...
	SingletonInstance(CEffectManager)
public:
	void Init();
	void Load(char* path);
	...
	void MainProc();
	bool AddEffect(LPOBJ lpObj,bool type,int index,int count,WORD value1,WORD value2,WORD value3,WORD value4,bool send = true);
	...
	DWORD GetViewSkillState(BYTE effect);
	void GCEffectStateSend(LPOBJ lpObj,BYTE state,BYTE effect);
	bool GetPeriodicCode(BYTE effect,WORD* group,WORD* value);
	void GCPeriodicEffectSend(LPOBJ lpObj,WORD group,WORD value,BYTE state,DWORD time,BYTE effect,bool send = false);
	void GCPartyEffectListSend(LPOBJ lpObj);
private:
	EFFECT_INFO m_EffectInfo[MAX_EFFECT];
};

#define gEffectManager SingNull(CEffectManager)