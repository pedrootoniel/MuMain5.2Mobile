...
void CItem::Convert(int index,BYTE Option1,BYTE Option2,BYTE Option3,BYTE NewOption,BYTE SetOption,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus) // OK
{
	...
	if(this->m_Index == GET_ITEM(13,19) || this->m_Index == GET_ITEM(14,64) || this->m_Index == GET_ITEM(14,223))
	{
		this->m_EventItem = 1;
	}
	else
	...
	if(this->m_Defense > 0)
	{
		...
		else
		{
			...
			else if(gWingOption->CheckWingByItem(this->m_Index) != 0)
			{
				this->m_Defense += gWingOption->GetWingDefense(this->m_Index,this->m_Level);
			}
			else if(gCustomWing->CheckCustomWingByItem(this->m_Index) != 0)
			{
...
void CItem::Value() // OK
{
	...
	else if(ItemInfo.Value > 0)
	{
		...
		if(this->m_Index >= GET_ITEM(14,0) && this->m_Index <= GET_ITEM(14,8))
		{
			...
			price *= (QWORD)(1 << this->m_Level);

			price *= (QWORD)this->m_Durability;
	...
	else
	{
		...
		if(this->m_NewOption != 0 && this->m_Index < GET_ITEM(12,0))
		{
			ItemLevel += 25;
		}
		...
		if(((this->m_Index/MAX_ITEM_TYPE) == 12 && ((this->m_Index > GET_ITEM(12,6) && this->m_Index < GET_ITEM(12,36)) || this->m_Index > GET_ITEM(12,43)) && this->m_Index != GET_ITEM(12,50) && (this->m_Index < GET_ITEM(12,262) || this->m_Index > GET_ITEM(12,265)) && gCustomWing->CheckCustomWingByItem(this->m_Index) == 0) || (this->m_Index/MAX_ITEM_TYPE) == 13 || (this->m_Index/MAX_ITEM_TYPE) == 15)
		{
			...
			price = ((this->m_Option3!=0)?(price+(price*this->m_Option3)):price);

			for(int n = 0; n < GetNewOptionCount(this->m_NewOption); n++)
			{
		...
		else
		{
			...
			if(this->m_Option3 != 0)
			{
				price += (((price*(60*this->m_Option3))/100));
			}
...
{
	if(this->m_Index == GET_ITEM(13,4) || this->m_Index == GET_ITEM(13,5)) // Dark Horse, Dark Reaven
	{
		this->m_LoadPetItemInfo = 1;

		this->m_PetItemLevel = PetLevel;

		this->m_PetItemLevel = ((this->m_PetItemLevel > gServerInfo->m_PetMaxLevel) ? gServerInfo->m_PetMaxLevel : this->m_PetItemLevel);

		this->m_PetItemExp = PetExp;

		this->m_PetItemExp = ((this->m_PetItemExp < gPetExperience[this->m_PetItemLevel]) ? gPetExperience[this->m_PetItemLevel] : this->m_PetItemExp);

		this->m_PetItemExp = ((this->m_PetItemExp >= gPetExperience[this->m_PetItemLevel+1]) ? gPetExperience[this->m_PetItemLevel] : this->m_PetItemExp);
...
bool CItem::AddPetItemExp(int amount) // OK
{
	...
	amount /= gServerInfo->m_PetExperienceRateDivisor;

	if(this->m_Index == GET_ITEM(13,4) || this->m_Index == GET_ITEM(13,5)) // Dark Horse, Dark Reaven
	{
		if(this->m_PetItemLevel >= gServerInfo->m_PetMaxLevel)
		{
			return 0;
		}

		if((this->m_PetItemExp+amount) >= gPetExperience[this->m_PetItemLevel+1])
		{
			this->m_PetItemLevel++;
			this->m_PetItemExp = gPetExperience[this->m_PetItemLevel];
			this->Convert(this->m_Index,this->m_Option1,this->m_Option2,this->m_Option3,this->m_NewOption,this->m_SetOption,this->m_JewelOfHarmonyOption,this->m_ItemOptionEx,this->m_SocketOption,this->m_SocketOptionBonus);
			this->PetValue();
			return 1;
		}
...
bool CItem::DecPetItemExp(int amount) // OK
{
	...
	if(this->m_Index == GET_ITEM(13,4) || this->m_Index == GET_ITEM(13,5)) // Dark Horse, Dark Reaven
	{
		if((this->m_PetItemExp-amount) < gPetExperience[this->m_PetItemLevel])
		{
			this->m_PetItemLevel--;
			this->m_PetItemExp = gPetExperience[this->m_PetItemLevel+1]-1;
			this->Convert(this->m_Index,this->m_Option1,this->m_Option2,this->m_Option3,this->m_NewOption,this->m_SetOption,this->m_JewelOfHarmonyOption,this->m_ItemOptionEx,this->m_SocketOption,this->m_SocketOptionBonus);
			this->PetValue();
			return 1;
		}
