...
void CCrywolfAltar::ResetAltarUserIndex(int AltarObjIndex,int Class) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpAltarInfo->m_UserIndex,1,0,0,0,0,0,gMessage->GetMessage(101),AltarNumber);
	}
...
bool CCrywolfAltar::SetAltarUserIndex(int AltarObjIndex,int Class,int UserIndex) // OK
{
	...
	if(gObj[lpAltarInfo->m_UserIndex].Connected == OBJECT_ONLINE)
	{
		gNotice->GCNoticeSend(UserIndex,1,0,0,0,0,0,gMessage->GetMessage(94),AltarNumber);
		return 0;
	}

	if((CurrentTime-lpAltarInfo->m_LastValidContractTime) < 10000)
	{
		gNotice->GCNoticeSend(UserIndex,1,0,0,0,0,0,gMessage->GetMessage(95),AltarNumber);
		return 0;
	}

	if(lpAltarInfo->m_ContractCount > MAX_CRYWOLF_ALTAR_CONTRACT) 
	{
		gNotice->GCNoticeSend(UserIndex,1,0,0,0,0,0,gMessage->GetMessage(96),AltarNumber);
		return 0;
	}

	if(abs(gObj[AltarObjIndex].X-gObj[UserIndex].X) > 0 || abs(gObj[AltarObjIndex].Y-gObj[UserIndex].Y) > 0)
	{
		gNotice->GCNoticeSend(UserIndex,1,0,0,0,0,0,gMessage->GetMessage(97),AltarNumber);
		return 0;
	}
	...
	gCrywolfUtil->SendCrywolfUserAnyMsg(0,gMessage->GetMessage(98),gObj[lpAltarInfo->m_UserIndex].Name,AltarNumber);

	gNotice->GCNoticeSend(UserIndex,1,0,0,0,0,0,gMessage->GetMessage(99));
	gNotice->GCNoticeSend(UserIndex,1,0,0,0,0,0,gMessage->GetMessage(100));
	return 1;
}
...
void CCrywolfAltar::SetAltarValidContract(int AltarObjIndex,int Class) // OK
{
	...
	{
		gNotice->GCNoticeSend(lpAltarInfo->m_UserIndex,1,0,0,0,0,0,gMessage->GetMessage(92),AltarNumber);

		gCrywolfUtil->SendCrywolfUserAnyMsg(0,gMessage->GetMessage(93),gObj[lpAltarInfo->m_UserIndex].Name,AltarNumber);
