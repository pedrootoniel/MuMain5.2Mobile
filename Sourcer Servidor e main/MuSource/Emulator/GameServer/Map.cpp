...
void CMap::Load(char* path,int map) // OK
{
	...
	{
		ErrorMessageBox("[%s] Error opening file.",path);
		return;
	}
	...
	{
		ErrorMessageBox("[%s] Error reading file header.",path);
		CloseHandle(file);
		return;
	}
	...
	{
		ErrorMessageBox("[%s] Error reading map width.",path);
		CloseHandle(file);
		return;
	}
	...
	{
		ErrorMessageBox("[%s] Error reading map height.",path);
		CloseHandle(file);
		return;
	}
	...
	{
		ErrorMessageBox("[%s] Invalid map dimensions.",path);
		CloseHandle(file);
		return;
	}
	...
	if(this->m_MapAttr == 0)
	{
		ErrorMessageBox("[%s] Error allocating memory for map.",path);
		CloseHandle(file);
		return;
	}
	...
	{
		ErrorMessageBox("[%s] Error reading map data.",path);
		CloseHandle(file);
		return;
	}
...
bool CMap::MoneyItemDrop(int money,int x,int y) // OK
{
	...
	for(int n=0;n < MAX_MAP_ITEM;n++)
	{
		if(this->m_Item[count].IsItem() == 0)
		{
			...
			this->m_Item[count].m_CreateTime = GetTickCount();
...
bool CMap::MonsterItemDrop(int index,int level,float dur,int x,int y,BYTE Option1,BYTE Option2,BYTE Option3,BYTE NewOption,BYTE SetOption,int aIndex,DWORD serial,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus,DWORD duration) // OK
{
	...
	for(int n=0;n < MAX_MAP_ITEM;n++)
	{
		if(this->m_Item[count].IsItem() == 0)
		{
			...
			this->m_Item[count].m_LootUserIndex = aIndex;
			this->m_ItemCount = (((++this->m_ItemCount)>=MAX_MAP_ITEM)?0:this->m_ItemCount);
			return 1;
		}
		else
...
bool CMap::CheckItemGive(int aIndex,int index) // OK
{
	...
	if(this->m_Item[index].m_LootUserIndex != aIndex && gItemLoot->CheckItemAllowLoot(this->m_Item[index].m_Index,this->m_Item[index].m_Level) == 0)
	{
		return 0;
	}
...
bool CMap::CheckWall(int sx1, int sy1, int sx2, int sy2) // OK
{
	int Index = (sy1 & 255) * 256 + (sx1 & 255);
	int nx1;
	int ny1;
	int d1;
	int d2;
...
bool CMap::PathFinding2(int sx, int sy, int tx, int ty, PATH_INFO* a) // OK
{
	...
	if (Success)
	{
		int PathNum = this->m_path->GetPath();

		if (PathNum > 1)
		{
			a->PathNum = PathNum;

			BYTE* x = this->m_path->GetPathX();
			BYTE* y = this->m_path->GetPathY();
...
bool CMap::PathFinding3(int sx, int sy, int tx, int ty, PATH_INFO* a) // OK
{
...
bool CMap::PathFinding4(int sx, int sy, int tx, int ty, PATH_INFO* a) // OK
{
