...
void gObjFirstProc() // OK
{
	...
	gCastleSiege->GDCastleSiegeSyncStateSend();
}
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	memset(lpObj->GoldenArcherTransaction,0,sizeof(lpObj->GoldenArcherTransaction));
	...
	lpObj->CsNpcType = 0;
	lpObj->CastleGateState = 0;
	lpObj->CastleSwitchIndex = -1;
	lpObj->CastleDefenseLevel = 0;
	lpObj->CastleRegenLevel = 0;
	...
	#if(GAMESERVER_UPDATE>=803)
	lpObj->CursedStone.Reset();
	#endif

	lpObj->WindowTimeOpen = false;

	#if(GAMESERVER_UPDATE>=702)
	lpObj->ArcaBattleGate = 0;
	lpObj->BootyDrop = false;
	#endif

	lpObj->AutoPotion = false;
	lpObj->AutoPotionTime = 0;
...
short gObjAddSearch(SOCKET socket,char* IpAddress) // OK
{
	...
	if(gObjTotalUser >= ((gServerInfo->m_ServerMaxUserNumber > 1000) ? 1000 : gServerInfo->m_ServerMaxUserNumber))
	{
...
bool gObjCheckResistance(LPOBJ lpObj,int type) // OK
{
	...
	if(lpObj->Authority >= AUTHORITY_GAME_MASTER && (lpObj->Inventory[INVENTORY_SLOT_RING1].m_Index == GET_ITEM(13,42) || lpObj->Inventory[INVENTORY_SLOT_RING2].m_Index == GET_ITEM(13,42)))
	{
		return 1;
	}
...
void gObjViewportUnionUpdate(int aIndex) // OK
{
	...
	gViewport->GCViewportSimpleUnionSend(&gObj[aIndex]);
}
//**************************************************************************//
// USER FUNCTIONS **********************************************************//
//**************************************************************************//
...
void gObjTeleportMagicUse(int aIndex,int x,int y) // OK
{
	...
	lpObj->PathCount = 0;
	lpObj->Teleport = 1;
	lpObj->ViewState = VIEW_STATE_TELEPORT;
	lpObj->X = x;
	lpObj->Y = y;
	lpObj->TX = x;
	lpObj->TY = y;
...
void gObjPlayerDiePenalty(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(exprate > 0)
	{
		...
		{
			experience = (DWORD)(gMasterSkillTree->m_MasterLevelExperienceTable[lpObj->MasterLevel]-gMasterSkillTree->m_MasterLevelExperienceTable[lpObj->MasterLevel-1]);
		}
		...
		if(experience > 0)
		{
			...
			{
				lpObj->MasterExperience -= experience;

				lpObj->MasterExperience = (DWORD)(lpObj->MasterExperience<gMasterSkillTree->m_MasterLevelExperienceTable[lpObj->MasterLevel-1])?gMasterSkillTree->m_MasterLevelExperienceTable[lpObj->MasterLevel-1]:lpObj->MasterExperience;

				if(lpObj->MasterExperience < 0)
				{
					lpObj->MasterExperience = 0;
				}

				gLog->Output(LOG_PENALTY,"[PenaltyMasterExpDecrease][%s][%s] - (Rate:%d, PKLevel:%d, Experience: %d)",lpObj->Account,lpObj->Name,lpObj->PKLevel,exprate,experience);
			}
	...
	if(gServerInfo->m_MoneyDeduceRate > 0 && lpObj->Money > 0)
	{
	...
	if(gMapManager->GetMapPKItemDrop(lpObj->Map) == 0 || droprate == 0)
	{
		return;
	}
	...
	if((GetLargeRand()%100) < droprate)
	{
		...
		for(int n = 0; n < gItemManager->GetInventoryMaxValue(lpObj); n++)
		{
		...
		if(RandomManager.GetRandomElement(&slot) != 0)
		{
			...
			{
				gLog->Output(LOG_PENALTY,"[PenaltyDropItem][%s][%s] - (Rate:%d, PKLevel:%d, Map:%d, X:%d, Y:%d) (Index: %04d, Level: %02d, Serial: %08X, Option1: %01d, Option2: %01d, Option3: %01d, NewOption: %03d, JewelOfHarmonyOption: %03d, ItemOptionEx: %03d, SocketOption: %03d,%03d,%03d,%03d,%03d)",lpObj->Account,lpObj->Name,droprate,lpObj->PKLevel,lpObj->Map,lpObj->X,lpObj->Y,lpItem->m_Index,lpItem->m_Level,lpItem->m_Serial,lpItem->m_Option1,lpItem->m_Option2,lpItem->m_Option3,lpItem->m_NewOption,lpItem->m_JewelOfHarmonyOption,lpItem->m_ItemOptionEx,lpItem->m_SocketOption[0],lpItem->m_SocketOption[1],lpItem->m_SocketOption[2],lpItem->m_SocketOption[3],lpItem->m_SocketOption[4]);
				pMsg.result = 1;
...
void gObjPlayerKiller(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		if(gCastleSiegeSync->GetState() == CS_STATE_START && lpObj->CastleJoinSide != 0)
		{
			return;
		}
		else if(lpObj->Y > 113)
...
int gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if(TargetGate == 97)
	{
		...
		{
			if(lpObj->CastleJoinSide != 1 && gCastleSiege->CheckCastleTowerEnable() == 0)
			{
				goto ERROR_JUMP;
			}
	...
	if(map == MAP_ICARUS && lpObj->Inventory[INVENTORY_SLOT_WING].IsItem() == 0 && lpObj->Inventory[INVENTORY_SLOT_HELPER].m_Index != GET_ITEM(13,3) && lpObj->Inventory[INVENTORY_SLOT_HELPER].m_Index != GET_ITEM(13,37))
	{
		gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage->GetMessage(275));
		goto ERROR_JUMP;
	}
...
bool gObjBackSpring(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Class >= 523 && lpObj->Class <= 528) // Trap, Castle Gate, Castle Statue
	{
		return 0;
	}

	if(lpObj->Class >= 598 && lpObj->Class <= 602) // Fire Tower, Water Tower, Earth Tower, Wind Tower, Darkness Tower
	{
		return 0;
	}

	if(lpObj->Class >= 627 && lpObj->Class <= 631) // Cursed Fire Tower, Cursed Water Tower, Cursed Earth Tower, Cursed Wind Tower, Cursed Darkness Tower
	{
		return 0;
	}

	if(lpObj->Class >= 131 && lpObj->Class <= 134) // Castle Gate, Statue of Saint
	{
		return 0;
	}
	...
	if(lpObj->Class == 348) // Tanker
	{
		return 0;
	}

	if(lpObj->Class == 275 || lpObj->Class == 657) // Kundun, Golden Kundun
	{
		return 0;
	}

	if(lpObj->Class == 673) // Lord Silvester
	{
		return 0;
	}

	if(lpObj->Class == 459 || lpObj->Class == 460 || lpObj->Class == 461 || lpObj->Class == 462) // Selupan, Spider Eggs
	{
		return 0;
	}
...
bool BackSpringCheck(int & x,int & y,int & dir,BYTE map)
{
	...
	ty = ty+RoadPathTable[dir+1];

	BYTE attr = gMap[map].GetAttr(tx,ty);
...
bool gObjBackSpring2(LPOBJ lpObj,LPOBJ lpTargetObj,int count)
{
	...
	if(lpObj->Class == 287 || lpObj->Class == 286) // Archer, Spearman
	{
		return 0;
	}

	if(lpObj->Class == 278) // Life Stone
	{
		return 0;
	}
