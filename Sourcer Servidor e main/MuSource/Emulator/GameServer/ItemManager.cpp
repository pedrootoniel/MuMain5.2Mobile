...
void CItemManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			while(true)
			{
				...
				info.Index = SafeGetItem(GET_ITEM(section,lpReadFile->GetNumber()));

				info.Slot = lpReadFile->GetAsNumber();

				info.Skill = lpReadFile->GetAsNumber();

				info.Width = lpReadFile->GetAsNumber();

				info.Height = lpReadFile->GetAsNumber();

				info.HaveSerial = lpReadFile->GetAsNumber();

				info.HaveOption = lpReadFile->GetAsNumber();

				info.DropItem = lpReadFile->GetAsNumber();
				...
				if(section >= 0 && section <= 5)
				{
					...
					info.MagicDurability = lpReadFile->GetAsNumber();

					info.MagicDamageRate = lpReadFile->GetAsNumber();

					info.RequireLevel = lpReadFile->GetAsNumber();

					info.RequireStrength = lpReadFile->GetAsNumber();

					info.RequireDexterity = lpReadFile->GetAsNumber();

					info.RequireEnergy = lpReadFile->GetAsNumber();

					info.RequireVitality = lpReadFile->GetAsNumber();
				...
				else if(section >= 6 && section <= 11)
				{
					...
					if(section == 6)
					{
						...
						info.DefenseSuccessRate = lpReadFile->GetAsNumber();
					}
					...
					else if(section == 11)
					{
						...
						info.WalkSpeed = lpReadFile->GetAsNumber();
					}
				...
				else if(section == 12)
				{
					...
					info.BuyMoney = lpReadFile->GetAsNumber();
				}
				else if(section == 13)
				{
					info.Level = lpReadFile->GetAsNumber();
				...
				{
					info.SetAttr = lpReadFile->GetAsNumber();
				}
	...
	Console(2,"[ItemManager] %d Items Loaded",this->m_ItemInfo.size());
}
...
void CItemManager::GetItemBmd(char* path) // OK
{
	HANDLE file = CreateFile(path,GENERIC_WRITE,FILE_SHARE_READ,0,CREATE_ALWAYS,FILE_ATTRIBUTE_ARCHIVE,0);

	if(file == INVALID_HANDLE_VALUE)
	{
		LogAdd(LOG_RED,"[%s] Error opening file.",path);
		return;
	}

	BMD_INFO info[MAX_ITEM];
	...
	{
		if(this->GetInfo(n,&ItemInfo) != 0)
		{
			strcpy_s(info[n].Name,ItemInfo.Name);
			info[n].TwoHand = ItemInfo.TwoHand;
			info[n].Level = ItemInfo.Level;
			info[n].Slot = ItemInfo.Slot;
			info[n].Skill = ItemInfo.Skill;
			info[n].Width = ItemInfo.Width;
			info[n].Height = ItemInfo.Height;
			info[n].DamageMin = ItemInfo.DamageMin;
			info[n].DamageMax = ItemInfo.DamageMax;
			info[n].DefenseSuccessRate = ItemInfo.DefenseSuccessRate;
			info[n].Defense = ItemInfo.Defense;
			info[n].MagicDefense = ItemInfo.MagicDefense;
			info[n].AttackSpeed = ItemInfo.AttackSpeed;
			info[n].WalkSpeed = ItemInfo.WalkSpeed;
			info[n].Durability = ItemInfo.Durability;
			info[n].MagicDurability = ItemInfo.MagicDurability;
			info[n].MagicDamageRate = ItemInfo.MagicDamageRate;
			info[n].RequireStrength = ItemInfo.RequireStrength;
			info[n].RequireDexterity = ItemInfo.RequireDexterity;
			info[n].RequireEnergy = ItemInfo.RequireEnergy;
			info[n].RequireVitality = ItemInfo.RequireVitality;
			info[n].RequireLeadership = ItemInfo.RequireLeadership;
			info[n].RequireLevel = ItemInfo.RequireLevel;
			info[n].Value = ItemInfo.Value;
			info[n].BuyMoney = ItemInfo.BuyMoney;
			info[n].SetAttr = ItemInfo.SetAttr;
			for(int i=0;i < MAX_RESISTANCE_TYPE;i++){info[n].Resistance[i] = ItemInfo.Resistance[i];}
			for(int i=0;i < MAX_CLASS;i++){info[n].RequireClass[i] = ItemInfo.RequireClass[i];}
		}

		PacketArgumentDecrypt((char*)&info[n],(char*)&info[n],sizeof(BMD_INFO));
	}

	DWORD BytesRead = 0;

	if(WriteFile(file,&info,sizeof(info),&BytesRead,0) == 0)
	{
		LogAdd(LOG_RED,"[%s] Error writing information to file.",path);
		CloseHandle(file);
		return;
	}

	DWORD checksum = GenerateCheckSum((char*)info,sizeof(info),58097);

	if(WriteFile(file,&checksum,sizeof(checksum),&BytesRead,0) == 0)
	{
		LogAdd(LOG_RED,"[%s] Error writing checksum to file.",path);
		CloseHandle(file);
		return;
	}
	...
	LogAdd(LOG_BLUE,"[%s] Create successfully",path);
}
...
int CItemManager::GetItemSlot(int index) // OK
{
	ITEM_INFO ItemInfo;
...
int CItemManager::GetItemDurability(int index,int level,int NewOption,int SetOption) // OK
{
	...
	if(index == GET_ITEM(0,41)) // Minning Axe
	{
		return 0;
	}
...
{
	for (std::vector<ITEM_LEVEL_INFO>::iterator it = gItemLevel->m_ItemLevelInfo.begin(); it != gItemLevel->m_ItemLevelInfo.end(); it++)
	{
...
bool CItemManager::CheckItemMoveToInventory(LPOBJ lpObj,CItem* lpItem,int slot) // OK
{
	...
	if(slot == 0 && lpObj->Inventory[INVENTORY_SLOT_WEAPON2].IsItem() != 0)
	{
		if((lpItem->m_TwoHand != 0 && lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_Index != GET_ITEM(4,7)) || (lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_TwoHand != 0 && lpItem->m_Index != GET_ITEM(4,15)))
		{
			return 0;
		}
	}

	if(slot == 1 && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].IsItem() != 0)
	{
		if((lpItem->m_TwoHand != 0 && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index != GET_ITEM(4,15)) || (lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_TwoHand != 0 && lpItem->m_Index != GET_ITEM(4,7)))
		{
			return 0;
		}
	}
...
bool CItemManager::CheckItemMoveToVault(LPOBJ lpObj,CItem* lpItem,BYTE TargetFlag) // OK
{
	...
	if(gServerInfo->m_VaultItemBlockLucky != 0 && lpItem->IsLuckyItem() != 0)
	{
		return 0;
	}
...
{
	if(gServerInfo->m_TradeItemBlock == 0 || lpObj->Authority == AUTHORITY_ADMINISTRATOR)
	{
		return 1;
	}
	...
	if(gServerInfo->m_TradeItemBlock380 != 0 && lpItem->Is380Item() != 0)
	{
		return 0;
	}
...
void CItemManager::DBItemByteConvert(BYTE* lpMsg,CItem* lpItem) // OK
{
	...
	if(lpItem->m_Index == GET_ITEM(13,19) || lpItem->m_Index == GET_ITEM(14,64) || lpItem->m_Index == GET_ITEM(14,223))
	{
		memset(lpMsg,0xFF,16);
		return;
	}
...
bool CItemManager::IsJewelItem(int index) // OK
{
	if(gCustomJewel->CheckCustomJewelByItem(index) != 0)
	{
		return 1;
	}
	...
	switch(index)
	{
		case GET_ITEM(12,15): // Jewel of Chaos
		case GET_ITEM(14,13): // Jewel of Bless
		case GET_ITEM(14,14): // Jewel of Soul
		case GET_ITEM(14,16): // Jewel of Life
		case GET_ITEM(14,22): // Jewel of Creation
		case GET_ITEM(14,31): // Jewel of Guardian
		case GET_ITEM(14,41): // Gemstone
		case GET_ITEM(14,42): // Jewel of Harmony
		case GET_ITEM(14,43): // Lower refining stone
		case GET_ITEM(14,44): // Higher refining stone
		case GET_ITEM(14,160): // Jewel of Extension
		case GET_ITEM(14,161): // Jewel of Elevation
			return 1;
	}
...
void CItemManager::DeleteInventoryItemCount(LPOBJ lpObj,int index,int level,int count) // OK
{
	...
	for(int n=0;n < MaxValue,count > 0;n++)
	{
		if(lpObj->Inventory[n].IsItem() != 0)
		{
			...
			{
				if(level == -1 || lpObj->Inventory[n].m_Level == level)
				{
					if(gItemStack->GetItemMaxStack(lpObj->Inventory[n].m_Index,lpObj->Inventory[n].m_Level) <= 0)
					{
						this->InventoryDelItem(lpObj->Index,n);
...
void CItemManager::DecreaseItemDur(LPOBJ lpObj,int slot,int dur) // OK
{
	...
	if(lpObj->Inventory[slot].m_Index == GET_ITEM(13,18) || lpObj->Inventory[slot].m_Index == GET_ITEM(13,29) || lpObj->Inventory[slot].m_Index == GET_ITEM(13,51) || lpObj->Inventory[slot].m_Index == GET_ITEM(14,19) || lpObj->Inventory[slot].m_Index == GET_ITEM(14,102) || lpObj->Inventory[slot].m_Index == GET_ITEM(14,109) || lpObj->Inventory[slot].m_Index == GET_ITEM(14,110)) // Invisibility Cloak,Armor of Guardsman,Scroll of Blood,Devil's Invitation,Gaion's Order,Complete Secromicon,Sign of Dimensions
	{
		this->InventoryDelItem(lpObj->Index,slot);
		this->GCItemDeleteSend(lpObj->Index,slot,1);
	}
	else if(lpObj->Inventory[slot].m_Durability > dur)
	{
...
int CItemManager::RepairItem(LPOBJ lpObj,CItem* lpItem,int slot,int type,DWORD* money) // OK
{
	...
	if(lpObj->Money == 0){GCMoneySend(lpObj->Index,lpObj->Money);}
...
BYTE CItemManager::MoveItemToPersonalShopFromInventory(LPOBJ lpObj,BYTE SourceSlot,BYTE TargetSlot,BYTE TargetFlag) // OK
{
	...
	if(lpObj->Inventory[SourceSlot].m_EventItem != 0)
	{
		return 0xFF;
	}
...
bool CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	{
		if(OBJECT_RANGE(lpObj->PartyNumber) != 0 && gServerInfo->m_PartyMoneyDistribute != 0)
		{
			int Money = (lpItem->m_BuyMoney/gParty->GetMemberCount(lpObj->PartyNumber));
			...
			for(int n=0;n < MAX_PARTY_USER;n++)
			{
				...
				if(gObjCheckMaxMoney(bIndex,Money) == 0)
				{
				...
				pMsg.ItemInfo[0] = SET_NUMBERHB(SET_NUMBERHW(lpTarget->Money));
				pMsg.ItemInfo[1] = SET_NUMBERLB(SET_NUMBERHW(lpTarget->Money));
				pMsg.ItemInfo[2] = SET_NUMBERHB(SET_NUMBERLW(lpTarget->Money));
				pMsg.ItemInfo[3] = SET_NUMBERLB(SET_NUMBERLW(lpTarget->Money));
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpItem->m_Index == GET_ITEM(13,20) && lpItem->m_Level != 0) // Starter Ring 40 && Starter Ring 80
	{
		if(lpObj->Level < 40 && lpItem->m_Level == 1)
		{
		...
		if(lpObj->Level < 80 && lpItem->m_Level == 2)
		{
	...
	else if(gItemBagManager->DropItemByItemIndex(lpItem->m_Index,lpItem->m_Level,lpObj,lpObj->Map,lpMsg->x,lpMsg->y) != 0)
	{
		this->InventoryDelItem(aIndex,lpMsg->slot);
	}
	...
	{
		if(gKalima->CreateKalimaPortal(lpObj,lpItem->m_Level,lpMsg->x,lpMsg->y) == 0)
		{
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	...
	{
		GCServerCommandSend(lpObj->Index,0,lpMsg->x,lpMsg->y,1);
		this->InventoryDelItem(aIndex,lpMsg->slot); 
	}
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	if(pMsg.result != 0xFF && lpMsg->TargetFlag == 0)
	{
		...
		this->GCItemListSend(aIndex);

		if(lpObj->Inventory[lpMsg->TargetSlot].m_IsPeriodicItem != 0)
		{
			gPeriodicItem->GCPeriodicItemSend(lpObj->Index,lpMsg->SourceSlot,lpMsg->TargetSlot,lpObj->Inventory[lpMsg->TargetSlot].m_PeriodicItemTime);
		}
	}
}
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		if(gKalima->CreateKalimaPortal(lpObj,gEventEntryLevel->GetKALevel(lpObj),lpObj->X,lpObj->Y) != 0)
		{
	...
	else if(lpItem->m_Index == GET_ITEM(13,97)) // MG Creation Card
	{
	...
	else if(lpItem->m_Index == GET_ITEM(13,98)) // DL Creation Card
	{
		if(gObjectManager->CharacterUseCreationCard(lpObj,lpItem) != 0)
		{
			this->InventoryDelItem(aIndex,lpMsg->SourceSlot);
	...
	else if(lpItem->m_Index == GET_ITEM(13,152) || lpItem->m_Index == GET_ITEM(13,156)) // Scroll of Green Oblivion
	{
		if(gMasterSkillTree->ResetMasterSkill(lpObj,1) != 0)
		{
			this->InventoryDelItem(aIndex,lpMsg->SourceSlot);
			this->GCItemDeleteSend(aIndex,lpMsg->SourceSlot,1);
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,153) || lpItem->m_Index == GET_ITEM(13,157)) // Scroll of Blue Oblivion
	{
		if(gMasterSkillTree->ResetMasterSkill(lpObj,2) != 0)
		{
			this->InventoryDelItem(aIndex,lpMsg->SourceSlot);
			this->GCItemDeleteSend(aIndex,lpMsg->SourceSlot,1);
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,154) || lpItem->m_Index == GET_ITEM(13,158)) // Scroll of Red Oblivion
	{
		if(gMasterSkillTree->ResetMasterSkill(lpObj,3) != 0)
		{
			this->InventoryDelItem(aIndex,lpMsg->SourceSlot);
			this->GCItemDeleteSend(aIndex,lpMsg->SourceSlot,1);
		}
	}
	else if(lpItem->m_Index == GET_ITEM(13,155) || lpItem->m_Index == GET_ITEM(13,159)) // Scroll of Gray Oblivion
	{
	...
	else if(lpItem->m_Index == GET_ITEM(14,7)) // Siege Potion
	{
		if(lpItem->m_Level == 0)
		{
			gEffectManager->AddEffect(lpObj,0,EFFECT_BLESS_POTION,gServerInfo->m_PotionOfBlessTimeConstA,gServerInfo->m_PotionOfBlessConstA,gServerInfo->m_PotionOfBlessConstB,0,0);
		}
		else
		{
			gEffectManager->AddEffect(lpObj,0,EFFECT_SOUL_POTION,gServerInfo->m_PotionOfSoulTimeConstA,gServerInfo->m_PotionOfSoulConstA,gServerInfo->m_PotionOfSoulConstB,0,0);
		}
	...
	{
		if(gEffectManager->CheckEffect(lpObj,EFFECT_POISON) != 0)
		{
			gEffectManager->DelEffect(lpObj,EFFECT_POISON);
			gSkillManager->GCSkillCancelSend(lpObj,SKILL_POISON);
		}

		if(gEffectManager->CheckEffect(lpObj,EFFECT_ICE) != 0)
		{
			gEffectManager->DelEffect(lpObj,EFFECT_ICE);
			gSkillManager->GCSkillCancelSend(lpObj,SKILL_ICE);
		}
...
void CItemManager::CGItemBuyRecv(PMSG_ITEM_BUY_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gShopManager->GetItemByIndex(lpObj->TargetShopNumber,&item,lpMsg->slot,&type) == 0)
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		return;
	}
	...
	if(type == 1)
	{
		if(lpObj->Coin1 < item.m_CoinValue)
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage->GetMessage(384),item.m_CoinValue);
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	}
	...
	{
		if(lpObj->Coin2 < item.m_CoinValue)
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage->GetMessage(385),item.m_CoinValue);
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	}
	...
	{
		if(lpObj->Coin3 < item.m_CoinValue)
		{
			gNotice->GCNoticeSend(aIndex,1,0,0,0,0,0,gMessage->GetMessage(386),item.m_CoinValue);
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	}
	...
	{
		if(lpObj->Money < (item.m_BuyMoney+tax))
		{
			DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			return;
		}
	...
	if(item.m_Index == GET_ITEM(13,4) || item.m_Index == GET_ITEM(13,5)) // Dark Horse, Dark Reaven
	{
		...
		if(type == 1)
		{
			gObjCoinSub(aIndex,item.m_CoinValue,0,0);
		}
		else if(type == 2)
		{
			gObjCoinSub(aIndex,0,item.m_CoinValue,0);
		}
		else if(type == 3)
		{
			gObjCoinSub(aIndex,0,0,item.m_CoinValue);
		}
		else
		{
...
void CItemManager::CGItemRepairRecv(PMSG_ITEM_REPAIR_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->slot == 0xFF)
	{
		...
		for(int n=0;n < MaxValue;n++)
		{
			...
			if(this->RepairItem(lpObj,&lpObj->Inventory[n],n,lpMsg->type,&pMsg.money) !=0)
			{
				DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
			}
		}
	...
	if(this->RepairItem(lpObj,&lpObj->Inventory[lpMsg->slot],lpMsg->slot,lpMsg->type,&pMsg.money) !=0)
	{
		DataSend(aIndex,(BYTE*)&pMsg,pMsg.header.size);
		gObjectManager->CharacterCalcAttribute(aIndex);
	}
...
void CItemManager::GCItemDropSend(int aIndex,BYTE slot) // OK
{
	PMSG_ITEM_DROP_SEND pMsg;
