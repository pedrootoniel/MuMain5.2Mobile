		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 2)
				{
					...
					{
						this->m_DevilSquareRewardExperience[level][n] = lpReadFile->GetAsNumber();
					}
				}
				else if(section == 3)
				{
					...
					for(int n=0;n < MAX_DS_RANK;n++)
					{
						this->m_DevilSquareRewardMoney[level][n] = lpReadFile->GetAsNumber();
					}
				}
				else
				{
...
void CDevilSquare::ProcState_EMPTY(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,gMessage->GetMessage(725),(lpLevel->MinutesLeft+1));
			}
		}
	}
	...
	{
		if(lpLevel->Level == 0){gNotice->GCNoticeSendToAll(0,0,0,0,0,0,gMessage->GetMessage(726));}
		this->SetState(lpLevel,DS_STATE_READY);
	}
}

void CDevilSquare::ProcState_READY(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	if(lpLevel->RemainTime > 0)
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				if(this->m_WarningTime > 1 && (lpLevel->MinutesLeft+1) == this->m_WarningTime)
				{
					return;
				}
...
void CDevilSquare::SetState_READY(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_BLACK,"[Devil Square] (%d) SetState READY",(lpLevel->Level+1));
}
...
void CDevilSquare::CheckSync(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	if(day > 0)
	{
		LogAdd(LOG_BLACK,"[Devil Square] (%d) Sync Start Time. [%d] day [%d] hour [%d] min remain",(lpLevel->Level+1),day,hr,min);
	}
	else if(hr > 0)
	{
		LogAdd(LOG_BLACK,"[Devil Square] (%d) Sync Start Time. [%d] hour [%d] min remain",(lpLevel->Level+1),hr,min);
	}
	else
	{
		LogAdd(LOG_BLACK,"[Devil Square] (%d) Sync Start Time. [%d] min remain",(lpLevel->Level+1),min);
	}
}
...
void CDevilSquare::GiveUserRewardItem(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	for(int n = 0; n < MAX_DS_USER; n++)
	{
		...
		gItemBagManager->DropItemBySpecialValue(ITEM_BAG_SPECIAL_DEVIL_SQUARE,lpLevel->Level,(lpLevel->User[n].Rank+1),&gObj[lpLevel->User[n].Index],gObj[lpLevel->User[n].Index].Map,gObj[lpLevel->User[n].Index].X,gObj[lpLevel->User[n].Index].Y);
	}
}
