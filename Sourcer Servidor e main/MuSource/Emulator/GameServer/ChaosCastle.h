...
#else
#define GET_CC_LEVEL(x) (((x)<MAP_CHAOS_CASTLE1)?-1:((x)>MAP_CHAOS_CASTLE6)?-1:((x)-MAP_CHAOS_CASTLE1))
#endif
...
class CChaosCastle
{
	...
	SingletonInstance(CChaosCastle)
public:
	void Init();
	void Load(char* path);
	void MainProc();
	...
	void ProcState_READY(CHAOS_CASTLE_LEVEL* lpLevel);
	...
	void SetState_READY(CHAOS_CASTLE_LEVEL* lpLevel);
	...
	void GiveUserRewardMoney(CHAOS_CASTLE_LEVEL* lpLevel);
	...
	void StageSpawn(CHAOS_CASTLE_LEVEL* lpLevel,int Stage);
	void SetMonster(CHAOS_CASTLE_LEVEL* lpLevel,int MonsterClass,int MaxRegenTime);
...
#define gChaosCastle SingNull(CChaosCastle)
...
#else
static const int gChaosCastleEnterMoney[MAX_CC_LEVEL] = {25000,80000,150000,250000,400000,650000};
#endif
