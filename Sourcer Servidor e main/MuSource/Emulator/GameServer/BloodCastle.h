...
#else
#define GET_BC_LEVEL(x) (((x)<MAP_BLOOD_CASTLE1)?-1:((x)>MAP_BLOOD_CASTLE7)?-1:((x)-MAP_BLOOD_CASTLE1))
#endif
...
class CBloodCastle
{
	...
	SingletonInstance(CBloodCastle)
public:
	void Init();
	void Load(char* path);
	void MainProc();
	...
	void ProcState_READY(BLOOD_CASTLE_LEVEL* lpLevel);
	...
	void SetState_READY(BLOOD_CASTLE_LEVEL* lpLevel);
	...
	void StageSpawn(BLOOD_CASTLE_LEVEL* lpLevel,int Stage);
	void SetMonster(BLOOD_CASTLE_LEVEL* lpLevel,int MonsterClass,int MaxRegenTime);
...
private:
	...
	int m_BloodCastleSaintStatue[MAX_BC_LEVEL][2];
};

#define gBloodCastle SingNull(CBloodCastle)
