...
void CDarkSpirit::MainProc() // OK
{
	...
	if(lpObj->Helper.Offline != 0)
	{
		...
		{
			this->m_ActionMode = DARK_SPIRIT_MODE_ATTACK_RANDOM;
		}
		else if(lpObj->Helper.DarkSpiritAuto != 0)
		{
			this->m_ActionMode = DARK_SPIRIT_MODE_ATTACK_RANDOM;
		}
		else if(lpObj->Helper.DarkSpiritAttack != 0)
		{
			this->m_ActionMode = DARK_SPIRIT_MODE_ATTACK_WITH_MASTER;
		}
		else
		{
			this->m_ActionMode = DARK_SPIRIT_MODE_NORMAL;
		}
	}
...
bool CDarkSpirit::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,BYTE action) // OK
{
	...
	if(this->MissCheck(lpObj,lpTarget,lpSkill,send,&miss) == 0)
	{
		return 1;
	}
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			#endif
			else if(gCastleSiegeSync->GetState() == CS_STATE_START && lpObj->Map == MAP_CASTLE_SIEGE && lpTarget->Map == MAP_CASTLE_SIEGE)
			{
	...
	if(damage > 0)
	{
		if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER && lpObj->Index != lpTarget->Index)
		{
			...
			if(lpObj->Map == MAP_CASTLE_SIEGE && gCastleSiegeSync->GetState() == CS_STATE_START && lpObj->CastleJoinSide != 0)
			{
				CheckSelfDefense = 0;
			}
...
{
	CItem* lpItem = &lpObj->Inventory[INVENTORY_SLOT_WEAPON2];

	gPetDamageTable->GetPetDamage(lpObj,5,&damage);
...
{
	CItem* lpItem = &lpObj->Inventory[INVENTORY_SLOT_WEAPON1];
