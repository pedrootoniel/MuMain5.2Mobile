...
void CDuel::CheckDuelScore(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gServerInfo->m_DuelArenaAnnounceSwitch == 2)
	{
		DUEL_ARENA_INFO* lpInfo = this->GetDuelArenaByGladiator(lpObj->Index);

		if(lpInfo != 0)
		{
			gNotice->GCNoticeSendToAll(0,0,0,0,0,0,gMessage->GetMessage(175),lpInfo->Index,lpObj->Name,lpObj->DuelScore,lpTarget->DuelScore,lpTarget->Name);
		}
	}
	...
	if(lpObj->DuelScore >= gServerInfo->m_DuelMaxScore)
	{
		...
		if(lpInfo != 0)
		{
			...
			if(gServerInfo->m_DuelArenaAnnounceSwitch != 0){gNotice->GCNoticeSendToAll(0,0,0,0,0,0,gMessage->GetMessage(176),lpObj->Name,lpTarget->Name);}
...
void CDuel::RespawnDuelUser(LPOBJ lpObj) // OK
{
	...
	if(OBJECT_RANGE(((lpInfo->Gladiator[0]==lpObj->Index)?lpInfo->Gladiator[0]:lpInfo->Gladiator[1])) == 0)
	{
		return;
	}

	LPOBJ lpTarget = ((lpInfo->Gladiator[0]==lpObj->Index)?(&gObj[lpInfo->Gladiator[0]]):(&gObj[lpInfo->Gladiator[1]]));
