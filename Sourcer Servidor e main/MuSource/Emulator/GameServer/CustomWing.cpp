...
void CCustomWing::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.OptionIndex1 = lpReadFile->GetAsNumber();

			info.OptionValue1 = lpReadFile->GetAsNumber();

			info.OptionIndex2 = lpReadFile->GetAsNumber();

			info.OptionValue2 = lpReadFile->GetAsNumber();

			info.OptionIndex3 = lpReadFile->GetAsNumber();

			info.OptionValue3 = lpReadFile->GetAsNumber();

			info.NewOptionIndex1 = lpReadFile->GetAsNumber();

			info.NewOptionValue1 = lpReadFile->GetAsNumber();

			info.NewOptionIndex2 = lpReadFile->GetAsNumber();

			info.NewOptionValue2 = lpReadFile->GetAsNumber();

			info.NewOptionIndex3 = lpReadFile->GetAsNumber();

			info.NewOptionValue3 = lpReadFile->GetAsNumber();

			info.NewOptionIndex4 = lpReadFile->GetAsNumber();

			info.NewOptionValue4 = lpReadFile->GetAsNumber();

			info.ModelType = lpReadFile->GetAsNumber();

			strcpy_s(info.ModelName,lpReadFile->GetAsString());
