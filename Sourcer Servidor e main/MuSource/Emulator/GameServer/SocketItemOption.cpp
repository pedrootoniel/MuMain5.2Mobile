		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					info.SubIndex = lpReadFile->GetAsNumber();
					...
					{
						info.ValueTable[n] = lpReadFile->GetAsNumber();
					}
				...
				else if(section == 1)
				{
					...
					info.MinItemSection = lpReadFile->GetAsNumber();

					info.MaxItemSection = lpReadFile->GetAsNumber();
					...
					for(int n=0;n < MAX_SOCKET_ITEM_OPTION_TABLE;n++)
					{
						info.RequireOptionTypeTable[n] = lpReadFile->GetAsNumber();
					}
				...
				else if(section == 2)
				{
					...
					{
						info.RequireOptionTypeCountTable[n] = lpReadFile->GetAsNumber();
					}
