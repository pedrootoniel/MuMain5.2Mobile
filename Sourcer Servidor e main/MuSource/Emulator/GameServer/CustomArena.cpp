		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 1)
				{
					...
					strcpy_s(this->m_CustomArenaInfo[index].RuleInfo.Name,lpReadFile->GetAsString());

					this->m_CustomArenaInfo[index].RuleInfo.AlarmTime = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.StandTime = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.EventTime = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.CloseTime = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.StartGate = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.FinalGate = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MinUser = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MaxUser = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MaxDeath = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MinLevel = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MaxLevel = lpReadFile->GetAsNumber();

					#if(GAMESERVER_UPDATE>=401)

					this->m_CustomArenaInfo[index].RuleInfo.MinMasterLevel = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MaxMasterLevel = lpReadFile->GetAsNumber();
					...
					this->m_CustomArenaInfo[index].RuleInfo.MinReset = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MaxReset = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MinMasterReset = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.MaxMasterReset = lpReadFile->GetAsNumber();

					this->m_CustomArenaInfo[index].RuleInfo.RewardBagIndex = lpReadFile->GetAsNumber();

					for(int n=0;n < MAX_CLASS;n++){this->m_CustomArenaInfo[index].RuleInfo.RequireClass[n] = lpReadFile->GetAsNumber();}
				}
				else
				{
					break;
...
void CCustomArena::SetState_CLEAN(CUSTOM_ARENA_INFO* lpInfo) // OK
{
	...
	for(int n=0;n < MAX_CUSTOM_ARENA_USER;n++)
	{
		if(OBJECT_RANGE(lpInfo->User[n].Index) != 0)
		{
			...
			gItemBagManager->DropItemBySpecialValue(((lpInfo->RuleInfo.RewardBagIndex==-1)?ITEM_BAG_SPECIAL_CUSTOM_ARENA:lpInfo->RuleInfo.RewardBagIndex),lpInfo->Index, lpInfo->User[n].Rank, &gObj[lpInfo->User[n].Index], gObj[lpInfo->User[n].Index].Map, gObj[lpInfo->User[n].Index].X, gObj[lpInfo->User[n].Index].Y);
