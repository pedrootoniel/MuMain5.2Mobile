	...
	try
	{
		while(true)
		{
			...
			for(int n=0;n < MAX_SET_ITEM_OPTION_TABLE;n++)
			{
				...
				{
					info.OptionTable[n][i].Index = lpReadFile->GetAsNumber();
					info.OptionTable[n][i].Value = lpReadFile->GetAsNumber();
				}
			}

			for(int o=0;o < MAX_SET_ITEM_FULL_OPTION_TABLE;o++)
			{
				info.FullOptionTable[o].Index = lpReadFile->GetAsNumber();
				info.FullOptionTable[o].Value = lpReadFile->GetAsNumber();
			}
...
int CSetItemOption::GetInventorySetItemOptionCount(LPOBJ lpObj,int index) // OK
{
	...
	for(int n=0;n < INVENTORY_WEAR_SIZE;n++)
	{
		...
		if(n == 0 || n == 1)
		{
			...
			else if(WeaponOptionIndex == OptionIndex && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_Index == lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_Index) 
			{
				continue;
			}
		}
		else if(n == 10 || n == 11)
		{
			...
			else if(RingOptionIndex == OptionIndex && lpObj->Inventory[INVENTORY_SLOT_RING1].m_Index == lpObj->Inventory[INVENTORY_SLOT_RING2].m_Index) 
			{
				continue;
			}
		}
...
void CSetItemOption::InsertOption(LPOBJ lpObj,int index,int value,bool flag) // OK
{
	...
	switch(index)
	{
		...
		case SET_ITEM_OPTION_MUL_TWO_HAND_PHYSI_DAMAGE:
			if((lpObj->Inventory[INVENTORY_SLOT_WEAPON1].IsItem() != 0 && lpObj->Inventory[INVENTORY_SLOT_WEAPON1].m_TwoHand != 0) || (lpObj->Inventory[INVENTORY_SLOT_WEAPON2].IsItem() != 0 && lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_TwoHand != 0))
			{
