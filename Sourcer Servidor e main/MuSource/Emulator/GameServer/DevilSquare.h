...
class CDevilSquare
{
	...
	SingletonInstance(CDevilSquare)
public:
	void Init();
	void Load(char* path);
	void MainProc();
	...
	void ProcState_READY(DEVIL_SQUARE_LEVEL* lpLevel);
	...
	void SetState_READY(DEVIL_SQUARE_LEVEL* lpLevel);
	...
	void StageSpawn(DEVIL_SQUARE_LEVEL* lpLevel,int Stage);
	void SetMonster(DEVIL_SQUARE_LEVEL* lpLevel,int MonsterClass,int MaxRegenTime);
...
#define gDevilSquare SingNull(CDevilSquare)