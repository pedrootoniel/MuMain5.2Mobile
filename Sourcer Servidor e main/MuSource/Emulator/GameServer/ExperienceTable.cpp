...
void CExperienceTable::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.MinMasterLevel = lpReadFile->GetAsNumber();

			info.MaxMasterLevel = lpReadFile->GetAsNumber();
			...
			info.MinMasterReset = lpReadFile->GetAsNumber();

			info.MaxMasterReset = lpReadFile->GetAsNumber();
