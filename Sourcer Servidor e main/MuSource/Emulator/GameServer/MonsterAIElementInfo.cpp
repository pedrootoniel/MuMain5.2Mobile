...
void CMonsterAIElementInfo::ApplyElementMove(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	if(lpMonsterAIState->m_TransitionType == 2 && gMonsterAIUtil->GetXYToChase(lpObj) != 0)
	{
		gMonsterAIUtil->FindPathToMoveMonster(lpObj,lpObj->MTX,lpObj->MTY,5,1);
		return;
	}

	if(lpMonsterAIState->m_TransitionType != 2 && gMonsterAIUtil->GetXYToPatrol(lpObj) != 0)
	{
		gMonsterAIUtil->FindPathToMoveMonster(lpObj,lpObj->MTX,lpObj->MTY,5,1);
		return;
	}
...
void CMonsterAIElementInfo::ApplyElementMoveTarget(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	if(gMonsterAIElement->m_MonsterAIMovePath[lpObj->Map].m_loaded == 0)
	{
		lpObj->PathStartEnd = gMonsterAIUtil->FindPathToMoveMonster(lpObj,PathX,PathY,5,0);
		return;
	}

	gMonsterAIElement->m_MonsterAIMovePath[lpObj->Map].GetMonsterNearestPath(lpObj->X,lpObj->Y,PathX,PathY,&PathX,&PathY,(int)sqrt(pow(((float)lpObj->X-(float)PathX),2)+pow(((float)lpObj->Y-(float)PathY),2)));

	lpObj->PathStartEnd = gMonsterAIUtil->FindPathToMoveMonster(lpObj,PathX,PathY,7,0);
...
void CMonsterAIElementInfo::ApplyElementGroupMove(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	int LeaderIndex = gMonsterAIGroup->FindGroupLeader(lpObj->GroupNumber);
	...
	if(OBJECT_RANGE(LeaderIndex) != 0 && gObj[LeaderIndex].Live != 0 && gObjCalcDistance(lpObj,&gObj[LeaderIndex]) > 6)
	{
		...
		if(gMonsterAIUtil->GetXYToChase(lpObj) != 0)
		{
			gMonsterAIUtil->FindPathToMoveMonster(lpObj,lpObj->MTX,lpObj->MTY,5,1);
		}
	}
...
void CMonsterAIElementInfo::ApplyElementAttack(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	if(gMonsterSkillManager->CheckMonsterSkill(lpObj->Class,0) == 0)
	{
		...
		gMonster->MonsterSkillAttack(lpObj,&gObj[lpObj->TargetNumber]);
	...
	else
	{
		...
		else
		{
			...
			gMonsterSkillManager->UseMonsterSkill(aIndex,lpObj->TargetNumber,0,-1,0);
...
void CMonsterAIElementInfo::ApplyElementAttackPenetration(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	gMonsterSkillManager->UseMonsterSkill(aIndex,lpObj->TargetNumber,2,-1,0);
...
void CMonsterAIElementInfo::ApplyElementAvoid(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	if(gMonsterAIUtil->GetXYToEascape(lpObj) == 0)
	{
		return;
	}
...
void CMonsterAIElementInfo::ApplyElementHealSelf(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	CMonsterSkillUnitInfo* lpMonsterSkillUnitInfo = gMonsterSkillManager->FindMonsterSkillUnitInfo(aIndex,21);
...
void CMonsterAIElementInfo::ApplyElementHealGroup(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	CMonsterAIGroupMember* lpMonsterAIGroupMember = gMonsterAIGroup->FindGroupMemberToHeal(aIndex,lpObj->GroupNumber,lpObj->GroupMemberGuid,6,lpMonsterAIState->m_TransitionValue);
...
void CMonsterAIElementInfo::ApplyElementSupportHeal(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	bIndex = gMonsterAIUtil->FindMonsterToHeal(&gObj[aIndex],lpMonsterAIState->m_TransitionValue,6);
...
void CMonsterAIElementInfo::ApplyElementSpecialSommon(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	CMonsterAIGroupMember* lpMonsterAIGroupMember = gMonsterAIGroup->FindGroupMemberToSommon(aIndex,lpObj->GroupNumber,lpObj->GroupMemberGuid);
	...
	CMonsterSkillUnitInfo* lpMonsterSkillUnitInfo = gMonsterSkillManager->FindMonsterSkillUnitInfo(aIndex,30);
...
void CMonsterAIElementInfo::ApplyElementSkillAttack(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	if(gMonsterSkillManager->CheckMonsterSkill(lpObj->Class,0) != 0 && (GetLargeRand()%100) < this->m_TargetType)
	{
		lpObj->Dir = GetPathPacketDirPos((gObj[lpObj->TargetNumber].X-lpObj->X),(gObj[lpObj->TargetNumber].Y-lpObj->Y));
		gMonsterSkillManager->UseMonsterSkill(aIndex,lpObj->TargetNumber,0,-1,0);
		lpObj->ActionState.Attack = 0;
	...
	if(gMonsterSkillManager->CheckMonsterSkill(lpObj->Class,1) != 0 && (GetLargeRand()%100) < this->m_X)
	{
		lpObj->Dir = GetPathPacketDirPos((gObj[lpObj->TargetNumber].X-lpObj->X),(gObj[lpObj->TargetNumber].Y-lpObj->Y));
		gMonsterSkillManager->UseMonsterSkill(aIndex,lpObj->TargetNumber,1,-1,0);
		lpObj->ActionState.Attack = 0;
		return;
	}

	if(gMonsterSkillManager->CheckMonsterSkill(lpObj->Class,2) != 0 && (GetLargeRand()%100) < this->m_Y)
	{
		lpObj->Dir = GetPathPacketDirPos((gObj[lpObj->TargetNumber].X-lpObj->X),(gObj[lpObj->TargetNumber].Y-lpObj->Y));
		gMonsterSkillManager->UseMonsterSkill(aIndex,lpObj->TargetNumber,2,-1,0);
		lpObj->ActionState.Attack = 0;
...
void CMonsterAIElementInfo::ApplyElementAIChange(int aIndex,int bIndex,CMonsterAIState* lpMonsterAIState) // OK
{
	...
	gMonsterAIGroup->ChangeAIOrder(this->m_TargetType,this->m_X);
