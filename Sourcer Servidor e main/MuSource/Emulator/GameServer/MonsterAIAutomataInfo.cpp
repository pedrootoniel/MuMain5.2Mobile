...
CMonsterAIState* CMonsterAIAutomataInfo::RunAutomataInfo(int aIndex) // OK
{
	...
	if(OBJECT_RANGE(lpObj->TargetNumber) != 0 && gMonsterAIUtil->FindMonViewportObj(aIndex,lpObj->TargetNumber) == 0)
	{
	...
	for(int n=0;n < this->m_AIStateTransCount[lpObj->CurrentAIState];n++)
	{
		...
		switch(lpMonsterAIState->m_TransitionType)
		{
			...
			case MAI_STATE_TRANS_GROUP_SOMMON:
				...
				{
					if(gMonsterAIGroup->FindGroupMemberToSommon(aIndex,lpObj->GroupNumber,lpObj->GroupMemberGuid) != 0 && (GetLargeRand()%100) < lpMonsterAIState->m_TransitionRate)
					{
						lpObj->LastAutomataDelay = lpMonsterAIState->m_DelayTime;
						return lpMonsterAIState;
					}
			...
			case MAI_STATE_TRANS_GROUP_HEAL:
				if(lpObj->GroupNumber >= 0 && lpObj->GroupNumber < MAX_MONSTER_AI_GROUP)
				{
					if(gMonsterAIGroup->FindGroupMemberToHeal(aIndex,lpObj->GroupNumber,lpObj->GroupMemberGuid,6,lpMonsterAIState->m_TransitionValue) != 0 && (GetLargeRand()%100) < lpMonsterAIState->m_TransitionRate)
					{
						lpObj->LastAutomataDelay = lpMonsterAIState->m_DelayTime;
						return lpMonsterAIState;
					}
			...
			case MAI_STATE_TRANS_SUPPORT_HEAL:
				if(OBJECT_RANGE(gMonsterAIUtil->FindMonsterToHeal(lpObj,lpMonsterAIState->m_TransitionValue,6)) != 0)
				{
					if((GetLargeRand()%100) < lpMonsterAIState->m_TransitionRate)
					{
						lpObj->LastAutomataDelay = lpMonsterAIState->m_DelayTime;
