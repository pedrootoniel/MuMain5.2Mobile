...
void CChaosBox::ArcaBattleRegisterMark(LPOBJ lpObj) // OK
{
	...
	if(gArcaBattle->GetState() != AB_STATE_EMPTY)
	{
	...
	gArcaBattle->GDArcaBattleRegisterMarkSaveSend(lpObj->Index,SignOfLord);

	gLog->Output(LOG_CHAOS_MIX,"[ArcaBattleRegisterMark][Success][%s][%s] - (SignOfLord: %d)",lpObj->Account,lpObj->Name,SignOfLord);
...
void CChaosBox::PentagramJewelUpgradeLevelMix(LPOBJ lpObj,BYTE info) // OK
{
	...
	else
	{
		...
		else
		{
			...
			this->GCChaosMixSend(lpObj->Index,227,&item);
