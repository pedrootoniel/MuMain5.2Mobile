...
#define GET_IT_LEVEL(x) (((x)<MAP_ILLUSION_TEMPLE1)?-1:((x)>MAP_ILLUSION_TEMPLE5)?-1:((x)-MAP_ILLUSION_TEMPLE1))
#endif
...
class CIllusionTemple
{
	...
	SingletonInstance(CIllusionTemple)
public:
	void Init();
	void Load(char* path);
	void MainProc();
	...
	void ProcState_READY(ILLUSION_TEMPLE_LEVEL* lpLevel);
	...
	void SetState_READY(ILLUSION_TEMPLE_LEVEL* lpLevel);
	...
	void StageSpawn(ILLUSION_TEMPLE_LEVEL* lpLevel,int Stage);
	void SetMonster(ILLUSION_TEMPLE_LEVEL* lpLevel,int MonsterClass,int MaxRegenTime);
...
#define gIllusionTemple SingNull(CIllusionTemple)