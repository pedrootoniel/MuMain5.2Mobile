...
void CItemOption::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.OptionIndex = lpReadFile->GetAsNumber();

			info.OptionValue = lpReadFile->GetAsNumber();
	...
	for(int n=0;n < MAX_SPECIAL;n++)
	{
		...
		switch(index)
		{
			...
			case ITEM_OPTION_ADD_COMBO_EXPERIENCE_RATE:
				if(lpObj->Inventory[INVENTORY_SLOT_HELPER].IsItem() != 0 && (lpObj->Inventory[INVENTORY_SLOT_HELPER].m_Index == GET_ITEM(13,80) || lpObj->Inventory[INVENTORY_SLOT_HELPER].m_Index == GET_ITEM(13,123)))
				{
					lpObj->ExperienceRate += value;
					lpObj->MasterExperienceRate += value;
				}
