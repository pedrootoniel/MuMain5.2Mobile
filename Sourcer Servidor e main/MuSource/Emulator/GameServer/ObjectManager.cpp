...
void CObjectManager::ObjectMsgProc(LPOBJ lpObj) // OK
{
	for(int n=0;n < MAX_MONSTER_SEND_MSG;n++)
	{
		if(gSMMsg[lpObj->Index][n].MsgCode != -1 && GetTickCount() > ((DWORD)gSMMsg[lpObj->Index][n].MsgTime))
		{
			...
			{
				gMonster->ObjectStateProc(lpObj,gSMMsg[lpObj->Index][n].MsgCode,gSMMsg[lpObj->Index][n].SendUser,gSMMsg[lpObj->Index][n].SubCode);
...
void CObjectManager::ObjectSetStateProc() // OK
{
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Type == OBJECT_MONSTER)
		{
			...
			if(lpObj->Type >= 586 && lpObj->Type <= 602 || lpObj->Type >= 627 && lpObj->Type <= 641)
			{
				continue;
			}
			...
			if(gMonster->RegenMonster(lpObj) == 0)
			{
				continue;
			}
...
void CObjectManager::ObjectStateProc(LPOBJ lpObj,int MessageCode,int aIndex,int SubCode) // OK
{
	...
	switch(MessageCode)
	{
		...
		case 4:
			...
			{
				gGuild->GuildWarEnd(lpObj->Guild,lpObj->Guild->WarEnemy);
			}
			break;
		case 10:
...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	if(OBJECT_RANGE(lpObj->PartyNumber) != 0)
	{
		...
		{
			if(gParty->GetMemberCount(lpObj->PartyNumber) <= ((gServerInfo->m_ReconnectTime>0)?1:2))
			{
	...
	{
		gGuild->GuildWarMasterClose(lpObj);
	}
...
{
	if(lpObj->Map == MAP_CASTLE_SIEGE && gCastleSiege->GetState() == CS_STATE_START)
	{
		gCastleSiege->GCCastleSiegeStartStateSend(lpObj->Index,1);
		gCastleSiege->GCCastleSiegeJoinSideSend(lpObj->Index);
		gCastleSiege->GCCastleSiegeLeftTimeSend(lpObj->Index);
	}
	else
	{
		gCastleSiege->GCCastleSiegeStartStateSend(lpObj->Index,0);
	}
...
void CObjectManager::CharacterCalcExperienceSplit(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag) // OK
{
	...
	gMonster->DelHitDamageUser(lpMonster);
...
void CObjectManager::CharacterCalcExperienceAlone(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag,bool type,int AttackDamage) // OK
{
	...
	else
	{
		...
		experience = (experience*gMapManager->GetMapMasterExperienceRate(lpObj->Map))/100;

		#endif

		experience = (experience*gBonusManager->GetBonusValue(lpObj,BONUS_INDEX_MASTER_EXPERIENCE_RATE,100))/100;
...
void CObjectManager::CharacterCalcExperienceParty(LPOBJ lpObj,LPOBJ lpMonster,int damage,int flag) // OK
{
	...
	for(int n=0;n < MAX_PARTY_USER;n++)
	{
		...
		else
		{
			...
			experience = (experience*gMapManager->GetMapMasterExperienceRate(lpTarget->Map))/100;

			#endif

			experience = (experience*gBonusManager->GetBonusValue(lpTarget,BONUS_INDEX_MASTER_EXPERIENCE_RATE,100))/100;
...
void CObjectManager::CharacterMakePreviewCharSet(int aIndex) // OK
{
	...
	lpObj->CharSet[1] = TempInventory[INVENTORY_SLOT_WEAPON1]%256;
	lpObj->CharSet[12] |= (TempInventory[INVENTORY_SLOT_WEAPON1]/16) & 0xF0;

	lpObj->CharSet[2] = TempInventory[INVENTORY_SLOT_WEAPON2]%256;
	lpObj->CharSet[13] |= (TempInventory[INVENTORY_SLOT_WEAPON2]/16) & 0xF0;

	lpObj->CharSet[3] |= (TempInventory[INVENTORY_SLOT_HELM] & 0x0F) << 4;
	lpObj->CharSet[9] |= (TempInventory[INVENTORY_SLOT_HELM] & 0x10) << 3;
	lpObj->CharSet[13] |= (TempInventory[INVENTORY_SLOT_HELM] & 0x1E0) >> 5;

	lpObj->CharSet[3] |= (TempInventory[INVENTORY_SLOT_ARMOR] & 0x0F);
	lpObj->CharSet[9] |= (TempInventory[INVENTORY_SLOT_ARMOR] & 0x10) << 2;
	lpObj->CharSet[14] |= (TempInventory[INVENTORY_SLOT_ARMOR] & 0x1E0) >> 1;

	lpObj->CharSet[4] |= (TempInventory[INVENTORY_SLOT_PANTS] & 0x0F) << 4;
	lpObj->CharSet[9] |= (TempInventory[INVENTORY_SLOT_PANTS] & 0x10) << 1;
	lpObj->CharSet[14] |= (TempInventory[INVENTORY_SLOT_PANTS] & 0x1E0) >> 5;

	lpObj->CharSet[4] |= (TempInventory[INVENTORY_SLOT_GLOVES] & 0x0F);
	lpObj->CharSet[9] |= (TempInventory[INVENTORY_SLOT_GLOVES] & 0x10);
	lpObj->CharSet[15] |= (TempInventory[INVENTORY_SLOT_GLOVES] & 0x1E0) >> 1;

	lpObj->CharSet[5] |= (TempInventory[INVENTORY_SLOT_BOOTS] & 0x0F) << 4;
	lpObj->CharSet[9] |= (TempInventory[INVENTORY_SLOT_BOOTS] & 0x10) >> 1;
	lpObj->CharSet[15] |= (TempInventory[INVENTORY_SLOT_BOOTS] & 0x1E0) >> 5;
	...
	{
		lpObj->CharSet[5] |= 4;
		lpObj->CharSet[9] |= TempInventory[INVENTORY_SLOT_WING]+1;
	}
	...
	{
		lpObj->CharSet[5] |= 12;
		lpObj->CharSet[9] |= TempInventory[INVENTORY_SLOT_WING]-35;
	}
	...
	{
		lpObj->CharSet[5] |= 12;
		lpObj->CharSet[17] |= (TempInventory[INVENTORY_SLOT_WING]-129) << 5;
	}
	else if(TempInventory[INVENTORY_SLOT_WING] >= 262 && TempInventory[INVENTORY_SLOT_WING] <= 265)
	{
		lpObj->CharSet[5] |= 8;
		lpObj->CharSet[16] |= (TempInventory[INVENTORY_SLOT_WING]-261) << 2;
	}
	...
	{
		lpObj->CharSet[5] |= 12;
		lpObj->CharSet[17] |= (gCustomWing->GetCustomWingIndex(GET_ITEM(12,TempInventory[INVENTORY_SLOT_WING]))+1) << 1;
	}
	...
	{
		lpObj->CharSet[5] |= 12;
		lpObj->CharSet[16] |= (TempInventory[INVENTORY_SLOT_WING]-35) << 2;
	}
	#endif
	#if(GAMESERVER_UPDATE>=401)
	else if(TempInventory[INVENTORY_SLOT_WING] == 41)
	...
	else if(TempInventory[INVENTORY_SLOT_HELPER] == 37)
	{
		...
		if((lpObj->Inventory[INVENTORY_SLOT_HELPER].m_NewOption & 1) != 0)
		{
			lpObj->CharSet[16] |= 1;
		}
		else if((lpObj->Inventory[INVENTORY_SLOT_HELPER].m_NewOption & 2) != 0)
		{
			lpObj->CharSet[16] |= 2;
		}
		else if((lpObj->Inventory[INVENTORY_SLOT_HELPER].m_NewOption & 4) != 0)
		{
			lpObj->CharSet[17] |= 1;
		}
	}
	...
	{
		lpObj->CharSet[16] |= (TempInventory[INVENTORY_SLOT_HELPER]-63)<<5;
	}
	#endif
	#if(GAMESERVER_UPDATE>=601)
...
bool CObjectManager::CharacterUsePotion(LPOBJ lpObj,CItem* lpItem) // OK
{
	...
	switch(lpItem->m_Index)
	{
		...
		case GET_ITEM(14,6):
			...
			break;
		case GET_ITEM(14,35):
			SDValue = (int)(((lpObj->MaxShield+lpObj->AddShield)*gServerInfo->m_SmallShieldPotionRate[lpObj->Class])/100);
			break;
		case GET_ITEM(14,36):
			SDValue = (int)(((lpObj->MaxShield+lpObj->AddShield)*gServerInfo->m_MidleShieldPotionRate[lpObj->Class])/100);
			break;
		case GET_ITEM(14,37):
			SDValue = (int)(((lpObj->MaxShield+lpObj->AddShield)*gServerInfo->m_LargeShieldPotionRate[lpObj->Class])/100);
			break;
		case GET_ITEM(14,38):
			HPValue = (int)(((lpObj->MaxLife+lpObj->AddLife)*gServerInfo->m_SmallCompoundPotionRate1[lpObj->Class])/100);
			SDValue = (int)(((lpObj->MaxShield+lpObj->AddShield)*gServerInfo->m_SmallCompoundPotionRate2[lpObj->Class])/100);
			break;
		case GET_ITEM(14,39):
			HPValue = (int)(((lpObj->MaxLife+lpObj->AddLife)*gServerInfo->m_MidleCompoundPotionRate1[lpObj->Class])/100);
			SDValue = (int)(((lpObj->MaxShield+lpObj->AddShield)*gServerInfo->m_MidleCompoundPotionRate2[lpObj->Class])/100);
			break;
		case GET_ITEM(14,40):
			HPValue = (int)(((lpObj->MaxLife+lpObj->AddLife)*gServerInfo->m_LargeCompoundPotionRate1[lpObj->Class])/100);
			SDValue = (int)(((lpObj->MaxShield+lpObj->AddShield)*gServerInfo->m_LargeCompoundPotionRate2[lpObj->Class])/100);
			break;
		case GET_ITEM(14,70):
			HPValue = (int)(((lpObj->MaxLife+lpObj->AddLife)*gServerInfo->m_EliteLifePotionRate[lpObj->Class])/100);
			break;
		case GET_ITEM(14,71):
			MPValue = (int)(((lpObj->MaxMana+lpObj->AddMana)*gServerInfo->m_EliteManaPotionRate[lpObj->Class])/100);
			break;
		case GET_ITEM(14,133):
			SDValue = (int)(((lpObj->MaxShield+lpObj->AddShield)*gServerInfo->m_EliteShieldPotionRate[lpObj->Class])/100);
			break;
	}
...
bool CObjectManager::CharacterUseJewelOfBles(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	if(slot != INVENTORY_SLOT_WEAPON1 && slot != INVENTORY_SLOT_WEAPON2 && slot != INVENTORY_SLOT_HELM && slot != INVENTORY_SLOT_ARMOR && slot != INVENTORY_SLOT_PANTS && slot != INVENTORY_SLOT_GLOVES && slot != INVENTORY_SLOT_BOOTS && slot != INVENTORY_SLOT_WING && slot != INVENTORY_SLOT_PENTAGRAM)
	{
		return 0;
	}
...
bool CObjectManager::CharacterUseCustomJewel(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	if((GetLargeRand()%100) < gCustomJewel->GetCustomJewelSuccessRate(lpObj->Inventory[SourceSlot].m_Index,lpObj->AccountLevel))
	{
		...
		#else
		lpItem->m_Level = (((lpItem->m_Level+lpInfo->Level)>13)?13:(lpItem->m_Level+lpInfo->Level));
		#endif
		...
		gCustomJewel->GetCustomJewelNewOption(lpItem,lpInfo->NewOption,1);

		gCustomJewel->GetCustomJewelSetOption(lpItem,lpInfo->SetOption,1);

		gCustomJewel->GetCustomJewelSocketOption(lpItem,lpInfo->SocketOption,1);
	...
	else
	{
		...
		gCustomJewel->GetCustomJewelNewOption(lpItem,lpInfo->NewOption,0);

		gCustomJewel->GetCustomJewelSetOption(lpItem,lpInfo->SetOption,0);

		gCustomJewel->GetCustomJewelSocketOption(lpItem,lpInfo->SocketOption,0);
...
bool CObjectManager::CharacterUseTalismanOfResurrection(LPOBJ lpObj,int SourceSlot) // OK
{
	...
	GCServerCommandSend(lpObj->Index,60,0,0,0);
	return 1;
}
...
bool CObjectManager::CharacterUseTalismanOfMobility(LPOBJ lpObj,int SourceSlot) // OK
{
	...
	GCServerCommandSend(lpObj->Index,61,0,0,0);
	return 1;
}
...
bool CObjectManager::CharacterUseCreationCard(LPOBJ lpObj,CItem* lpItem) // OK
{
	switch(lpItem->m_Index)
	{
		case GET_ITEM(13,97): // MG Creation Card
			if((lpObj->ClassCode & 4) == 0)
			{
				lpObj->ClassCode |= 4;
				GDCreationCardSaveSend(lpObj->Index,lpObj->ClassCode);
				return 1;
			}
			break;
		case GET_ITEM(13,98): // DL Creation Card
			if((lpObj->ClassCode & 2) == 0)
			{
				lpObj->ClassCode |= 2;
				GDCreationCardSaveSend(lpObj->Index,lpObj->ClassCode);
				return 1;
			...
			break;
		case GET_ITEM(14,91): // SU Creation Card
			if((lpObj->ClassCode & 1) == 0)
			{
				lpObj->ClassCode |= 1;
				GDCreationCardSaveSend(lpObj->Index,lpObj->ClassCode);
				return 1;
			...
			break;
		case GET_ITEM(14,169): // RF Creation Card
			if((lpObj->ClassCode & 8) == 0)
			{
				lpObj->ClassCode |= 8;
				GDCreationCardSaveSend(lpObj->Index,lpObj->ClassCode);
				return 1;
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	CItem* Pentagram = &lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM];
	#endif
	...
	lpObj->CurseDamageMin += lpObj->DrinkDamage;
	lpObj->CurseDamageMax += lpObj->DrinkDamage;
	...
	gDarkSpirit[lpObj->Index].Set(lpObj->Index,&lpObj->Inventory[INVENTORY_SLOT_WEAPON2]);

	lpObj->Resistance[RESISTANCE_ICE] = GET_MAX_RESISTANCE(Amulet->m_Resistance[RESISTANCE_ICE],Ring1->m_Resistance[RESISTANCE_ICE],Ring2->m_Resistance[RESISTANCE_ICE]);

	lpObj->Resistance[RESISTANCE_POISON] = GET_MAX_RESISTANCE(Amulet->m_Resistance[RESISTANCE_POISON],Ring1->m_Resistance[RESISTANCE_POISON],Ring2->m_Resistance[RESISTANCE_POISON]);

	lpObj->Resistance[RESISTANCE_LIGHTNING] = GET_MAX_RESISTANCE(Amulet->m_Resistance[RESISTANCE_LIGHTNING],Ring1->m_Resistance[RESISTANCE_LIGHTNING],Ring2->m_Resistance[RESISTANCE_LIGHTNING]);

	lpObj->Resistance[RESISTANCE_FIRE] = GET_MAX_RESISTANCE(Amulet->m_Resistance[RESISTANCE_FIRE],Ring1->m_Resistance[RESISTANCE_FIRE],Ring2->m_Resistance[RESISTANCE_FIRE]);

	lpObj->Resistance[RESISTANCE_EARTH] = GET_MAX_RESISTANCE(Amulet->m_Resistance[RESISTANCE_EARTH],Ring1->m_Resistance[RESISTANCE_EARTH],Ring2->m_Resistance[RESISTANCE_EARTH]);

	lpObj->Resistance[RESISTANCE_WIND] = GET_MAX_RESISTANCE(Amulet->m_Resistance[RESISTANCE_WIND],Ring1->m_Resistance[RESISTANCE_WIND],Ring2->m_Resistance[RESISTANCE_WIND]);

	lpObj->Resistance[RESISTANCE_WATER] = GET_MAX_RESISTANCE(Amulet->m_Resistance[RESISTANCE_WATER],Ring1->m_Resistance[RESISTANCE_WATER],Ring2->m_Resistance[RESISTANCE_WATER]);
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	memcpy(lpObj->MarryName,lpMsg->MarryName,sizeof(lpObj->MarryName));
	...
	lpObj->DGCount = lpMsg->DGCount;
	lpObj->ITCount = lpMsg->ITCount;
	lpObj->IGCount = lpMsg->IGCount;
	...
	{
		if(lpObj->Class == CLASS_DW || lpObj->Class == CLASS_DK || lpObj->Class == CLASS_MG || lpObj->Class == CLASS_DL || lpObj->Class == CLASS_RF)
		{
			result = gGate->GetGate(17,&gate,&map,&x,&y,&dir,&level);
		}
		else if(lpObj->Class == CLASS_FE)
	...
	else if(lpObj->Map == MAP_CASTLE_SIEGE && gCastleSiegeSync->GetState() == CS_STATE_START)
	{
		result = gGate->GetGate(100,&gate,&map,&x,&y,&dir,&level);
	}
	#if(GAMESERVER_UPDATE>=201)
	...
	for(int n=0;n < MAX_EFFECT_LIST;n++)
	{
		...
		gEffectManager->AddEffect(lpObj,1,effect.m_index,effect.m_count,effect.m_value[0],effect.m_value[1],effect.m_value[2],effect.m_value[3],((GAMESERVER_UPDATE>=401)?1:0));
	}
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage,bool combo) // OK
{
	if(lpObj->Connected != OBJECT_ONLINE)
	{
		return;
	...
	{
		gMonster->SetHitDamage(lpTarget,SummonIndex,damage);
	}
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			if(gGuild->GuildWarCheck(&gObj[SummonIndex],lpTarget) == 0)
			{
			...
			{
				gMonster->SummonKill(lpTarget->Index);
			}
		}
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			if(lpTarget->Class == 275 || lpTarget->Class == 277 || lpTarget->Class == 278 || lpTarget->Class == 283 || lpTarget->Class == 286 || lpTarget->Class == 287 || lpTarget->Class == 288 || lpTarget->Class == 295 || lpTarget->Class == 340 || lpTarget->Class == 348 || lpTarget->Class == 349 || lpTarget->Class == 361 || lpTarget->Class == 362 || lpTarget->Class == 363 || lpTarget->Class == 524 || lpTarget->Class == 525 || lpTarget->Class == 527 || lpTarget->Class == 528 || lpTarget->Class == 605 || lpTarget->Class == 606 || lpTarget->Class == 607)
			{
				return;
			}
		}
