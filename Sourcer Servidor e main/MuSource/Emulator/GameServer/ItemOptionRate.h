...
#define MAX_ITEM_OPTION5_RATE 7
...
class CItemOptionRate
{
	...
	SingletonInstance(CItemOptionRate)
public:
	void Load(char* path);
	...
	void MakeLevelOption(int MinLevel,int MaxLevel,short* level);
	void MakeNewOption(int ItemIndex,int random,int value,BYTE* option);
	bool MakeSetOption(int ItemIndex,int value,BYTE* option);
	void MakeSocketSlot(int ItemIndex,int value,BYTE* option);
...
#define gItemOptionRate SingNull(CItemOptionRate)