...
CBloodCastle::CBloodCastle() // OK
{
	...
	memset(this->m_BloodCastleSaintStatue,0,sizeof(this->m_BloodCastleSaintStatue));
}
		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 2)
				{
					...
					this->m_BloodCastleExperienceTable[level][0] = lpReadFile->GetAsNumber();

					this->m_BloodCastleExperienceTable[level][1] = lpReadFile->GetAsNumber();
				}
				else if(section == 3)
				{
					if(strcmp("end",lpReadFile->GetAsString()) == 0)
					...
					this->m_BloodCastleMoneyTable[level][0] = lpReadFile->GetAsNumber();

					this->m_BloodCastleMoneyTable[level][1] = lpReadFile->GetAsNumber();
				}
				else if(section == 4)
				{
					if(strcmp("end",lpReadFile->GetAsString()) == 0)
					...
					this->m_BloodCastleNpcLife[level][0] = lpReadFile->GetAsNumber();

					this->m_BloodCastleNpcLife[level][1] = lpReadFile->GetAsNumber();
				}
				else if(section == 5)
				{
					if(strcmp("end",lpReadFile->GetAsString()) == 0)
					...
					this->m_BloodCastleSaintStatue[level][0] = lpReadFile->GetAsNumber();

					this->m_BloodCastleSaintStatue[level][1] = lpReadFile->GetAsNumber();
				}
				else
				{
					break;
...
void CBloodCastle::ProcState_EMPTY(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				gNotice->GCNoticeSendToAll(0,0,0,0,0,0,gMessage->GetMessage(727),(lpLevel->MinutesLeft+1));
			}
		}
	}
	...
	{
		this->SetState(lpLevel,BC_STATE_READY);
	}
}

void CBloodCastle::ProcState_READY(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
...
void CBloodCastle::ProcState_START(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	{
		this->NoticeSendToAll(lpLevel,0,gMessage->GetMessage(12),gObj[lpLevel->WinnerIndex].Name);
...
void CBloodCastle::SetState_READY(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	LogAdd(LOG_BLACK,"[Blood Castle] (%d) SetState READY",(lpLevel->Level+1));
}
...
void CBloodCastle::CheckSync(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	if(day > 0)
	{
		LogAdd(LOG_BLACK,"[Blood Castle] (%d) Sync Start Time. [%d] day [%d] hour [%d] min remain",(lpLevel->Level+1),day,hr,min);
	}
	else if(hr > 0)
	{
		LogAdd(LOG_BLACK,"[Blood Castle] (%d) Sync Start Time. [%d] hour [%d] min remain",(lpLevel->Level+1),hr,min);
	}
	else
	{
		LogAdd(LOG_BLACK,"[Blood Castle] (%d) Sync Start Time. [%d] min remain",(lpLevel->Level+1),min);
	}
}
...
void CBloodCastle::GiveUserRewardItem(BLOOD_CASTLE_LEVEL* lpLevel,int aIndex) // OK
{
	...
	gItemBagManager->DropItemBySpecialValue(ITEM_BAG_SPECIAL_BLOOD_CASTLE,lpLevel->Level,1,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
}
...
void CBloodCastle::SetGate(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	int index = gMonster->AddMonster(lpLevel->Map);
	...
	if(gMonster->SetMonster(index,131) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CBloodCastle::SetSaintStatue(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	int px = this->m_BloodCastleSaintStatue[lpLevel->Level][0];
	int py = this->m_BloodCastleSaintStatue[lpLevel->Level][1];
	...
	if(gMonster->SetMonster(index,(132+(GetLargeRand()%3))) == 0)
	{
		gObjDel(index);
		return;
	}
...
void CBloodCastle::StageSpawn(BLOOD_CASTLE_LEVEL* lpLevel,int Stage) // OK
{
	...
	{
		if(it->Event != 0 || it->Level != lpLevel->Level || it->Stage != Stage)
		{
			continue;
		}
...
void CBloodCastle::NpcAngelKing(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	{
		GCServerCommandSend(lpObj->Index,1,46,0,0);
		return;
	}

	if(this->GetState(level) != BC_STATE_START)
	{
		GCServerCommandSend(lpObj->Index,1,24,0,0);
		return;
	}
	...
	GCServerCommandSend(lpObj->Index,1,23,0,0);
}
...
void CBloodCastle::NpcAngelMessenger(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	int level = gEventEntryLevel->GetBCLevel(lpObj);

	if(BC_LEVEL_RANGE(level) == 0)
	{
		GCNewMessageSend(lpObj->Index,0,0,gMessage->GetMessage(7));
		return;
	}

	if(gItemManager->GetInventoryItemCount(lpObj,GET_ITEM(13,18),(level+1)) == 0 && gItemManager->GetInventoryItemCount(lpObj,GET_ITEM(13,47),0) == 0 && gServerInfo->m_BloodCastleNPCWithoutEntrance == 0)
	{
		GCServerCommandSend(lpObj->Index,1,21,0,0);
		return;
	}

	if(this->GetEnterEnabled(level) == 0)
	{
		GCServerCommandSend(lpObj->Index,1,20,0,0);
		return;
	}
...
void CBloodCastle::DropEventItem(int map,int number,int aIndex) // OK
{
	...
	gMap[map].m_Item[number].m_CreateTime = GetTickCount();
...
void CBloodCastle::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Class >= 132 && lpObj->Class <= 134 && lpObj->Index == this->m_BloodCastleLevel[level].SaintStatueIndex && this->m_BloodCastleLevel[level].Stage == 4)
	{
		...
		this->StageSpawn(&this->m_BloodCastleLevel[level],5);
		return;
	}
...
void CBloodCastle::CGBloodCastleEnterRecv(PMSG_BLOOD_CASTLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gServerInfo->m_BloodCastleMaxEntryCount[lpObj->AccountLevel] != 0 && lpObj->BCCount >= gServerInfo->m_BloodCastleMaxEntryCount[lpObj->AccountLevel])
	{
		GCNewMessageSend(aIndex,0,0,gMessage->GetMessage(713),gServerInfo->m_BloodCastleMaxEntryCount[lpObj->AccountLevel]);
		return;
	}
...
void CBloodCastle::ForceStart() // OK
{
