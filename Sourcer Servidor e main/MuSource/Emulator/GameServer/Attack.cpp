...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(OBJECT_RANGE(lpObj->SummonIndex) != 0)
	{
		gMonster->SummonSetEnemy(lpObj,lpTarget->Index);
	}
	...
	if(lpTarget->Type == OBJECT_MONSTER)
	{
		...
		{
			gMonster->ObjectStateProc(lpTarget,6,lpObj->Index,0);
			this->MissSend(lpObj,lpTarget,lpSkill,send,count);
			return 1;
		}
		...
		if(lpTarget->Class == 200 && lpSkill != 0)
		{
			gMonster->ObjectStateProc(lpTarget,7,lpObj->Index,0);
			this->MissSend(lpObj,lpTarget,lpSkill,send,count);
			return 1;
		}
	...
	if(lpObj->Type == OBJECT_USER && effect != 4)
	{
		...
		if(lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_Index >= GET_ITEM(2,0) && lpObj->Inventory[INVENTORY_SLOT_WEAPON2].m_Index < GET_ITEM(3,0))
		{
			if((GetLargeRand()%100) < (gMasterSkillTree->GetMasterSkillValue(lpObj,MASTER_SKILL_ADD_MACE_MASTERY)-lpTarget->ResistStunRate))
			{
				if(gEffectManager->CheckEffect(lpTarget,EFFECT_IRON_DEFENSE) == 0 && gEffectManager->CheckEffect(lpTarget,EFFECT_IRON_DEFENSE_IMPROVED) == 0)
...
bool CAttack::AttackElemental(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && (lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].IsItem() == 0 || lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].IsPentagramItem() == 0 || lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].m_IsValidItem == 0))
	{
		return 0;
	}
	...
	if(damage > 0)
	{
		gObjectManager->CharacterLifeCheck(lpObj,lpTarget,(damage-ShieldDamage),4,flag,effect,skill,ShieldDamage,combo);
	}
	else
	{
...
int CAttack::GetTargetElementalDefense(LPOBJ lpObj,LPOBJ lpTarget,WORD* effect) // OK
{
	...
	if(lpTarget->Type == OBJECT_USER)
	{
		defense += (((lpTarget->Inventory[INVENTORY_SLOT_PENTAGRAM].IsItem()==0)?0:lpTarget->Inventory[INVENTORY_SLOT_PENTAGRAM].m_Defense)*lpTarget->PentagramOption.MulPentagramDefense)/100;
...
int CAttack::GetAttackDamageElemental(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int AttackDamage,int TargetDefense) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		DamageMin += (((lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].IsItem()==0)?0:lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].m_DamageMin)*lpObj->PentagramOption.MulPentagramDamage)/100;
		DamageMax += (((lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].IsItem()==0)?0:lpObj->Inventory[INVENTORY_SLOT_PENTAGRAM].m_DamageMax)*lpObj->PentagramOption.MulPentagramDamage)/100;
		...
		DamageMin += (DamageMin*lpObj->EffectOption.AddElementalDamage[lpObj->ElementalAttribute])/100;
		DamageMax += (DamageMax*lpObj->EffectOption.AddElementalDamage[lpObj->ElementalAttribute])/100;
