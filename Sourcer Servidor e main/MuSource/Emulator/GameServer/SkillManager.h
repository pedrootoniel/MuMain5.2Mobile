...
class CSkillManager
{
	...
	SingletonInstance(CSkillManager)
public:
	void Load(char* path);
	...
	bool CheckSkillRange(LPOBJ lpObj,int index,int x,int y,int tx,int ty);
	...
	bool SkillFireSlash(int aIndex, int bIndex, CSkill* lpSkill,bool combo);
	...
	bool SkillDamageReflect(int aIndex,int bIndex,CSkill* lpSkill,bool combo);
...
#define gSkillManager SingNull(CSkillManager)