...
void CMonster::MainProc(LPOBJ lpObj) // OK
{
	...
	if(lpObj->Class == 200) // Soccer Ball
	{
	...
	if(lpObj->ActionState.Attack == 1)
	{
		...
		if(gMonsterSkillManager->CheckMonsterSkill(lpObj->Class,0) == 0)
		{
			...
			{
				this->MonsterMultilAttack(lpObj,0);
			}
			else
			{
				this->MonsterSkillAttack(lpObj,&gObj[lpObj->TargetNumber]);
			}
...
void CMonster::ActionProc(LPOBJ lpObj) // OK
{
	...
	if(lpObj->ActionState.Emotion == EMOTION_REST)
	{
		if(lpObj->Attribute != ATTRIBUTE_NONE)
		{
			...
			else if(lpObj->MoveRange > 0 && this->CheckImmobilize(lpObj) != 0)
			{
				if(lpObj->Attribute != ATTRIBUTE_SUMMONED)
				{
					this->GetXYToPatrol(lpObj);
				}
				else
				{
					this->GetXYToOwner(lpObj);
				}
			}
			
			this->FindTarget(lpObj);
...
void CMonster::ObjectStateProc(LPOBJ lpObj,int code,int aIndex,int SubCode) // OK
{
	switch(code)
	{
		case 0:
			this->ProcessHit(lpObj,aIndex);
			break;
		case 1:
			this->MonsterDie(lpObj,&gObj[aIndex]);
			break;
		case 2:
...
int CMonster::AddMonster(int map) // OK
{
...
bool CMonster::RegenMonster(LPOBJ lpObj) // OK
{
...
bool CMonster::SetMonster(int aIndex,int MonsterClass) // OK
{
...
bool CMonster::SetPosition(int aIndex,int index) // OK
{
	if(OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
	...
	if(CHECK_RANGE(index,MAX_MSB_MONSTER) == 0)
	{
		return 0;
	}

	MONSTER_SET_BASE_INFO* lpInfo = &gMonsterSetBase->m_MonsterSetBaseInfo[index];

	short x,y;

	if(gMonsterSetBase->GetPosition(index,lpInfo->Map,&x,&y) == 0)
	{
		return 0;
	}
...
int CMonster::SummonSetEnemy(LPOBJ lpObj,int aIndex) // OK
{
...
void CMonster::SummonKill(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
void CMonster::FindTarget(LPOBJ lpObj) // OK
{
...
void CMonster::TrapFindTarget(LPOBJ lpObj) // OK
{
	int IndexTable[MAX_VIEWPORT];
	int IndexCount = 0;
...
void CMonster::GetXYToPatrol(LPOBJ lpObj) // OK
{
	if(this->CheckImmobilize(lpObj) == 0)
	{
		return;
	}
	...
	for(int n=0;n < 10;n++)
	{
		...
		if(this->MoveCheck(lpObj,tx,ty) != 0)
		{
...
bool CMonster::GetXYToChase(LPOBJ lpObj) // OK
{
	if(this->CheckImmobilize(lpObj) == 0)
	{
		return 0;
	...
	if(this->CheckViewport(lpObj->Index,lpObj->TargetNumber) == 0)
	{
		return 0;
	}
	...
	{
		if(this->MoveCheck(lpObj,sx,sy) != 0)
		{
...
void CMonster::GetXYToOwner(LPOBJ lpObj) // OK
{
	if(OBJECT_RANGE(lpObj->SummonIndex) == 0)
	{
		return;
...
void CMonster::FindPathToMove(LPOBJ lpObj) // OK
{
...
bool CMonster::MoveCheck(LPOBJ lpObj,int tx,int ty) // OK
{
...
bool CMonster::CheckImmobilize(LPOBJ lpObj) // OK
{
...
bool CMonster::CheckViewport(int aIndex,int bIndex) // OK
{
...
void CMonster::InitHitDamage(LPOBJ lpObj) // OK
{
...
void CMonster::SetHitDamage(LPOBJ lpObj,int aIndex,int damage) // OK
{
	...
	if(CHECK_RANGE(HitDamageIndex,MAX_HIT_DAMAGE) != 0 || CHECK_RANGE((HitDamageIndex=this->DelHitDamageUser(lpObj)),MAX_HIT_DAMAGE) != 0)
	{
...
int CMonster::DelHitDamageUser(LPOBJ lpObj) // OK
{
	int HitDamageIndex = -1;
...
int CMonster::GetTopHitDamageUser(LPOBJ lpObj) // OK
{
	...
	for(int n=0;n < MAX_HIT_DAMAGE;n++)
	{
		...
		{
			if((PartyTopHitDamage=this->GetTopHitDamageParty(lpObj,gObj[lpObj->HitDamage[n].index].PartyNumber,&PartyTopHitDamageUser)) > TopHitDamage)
			{
...
int CMonster::GetTopHitDamageParty(LPOBJ lpObj,int PartyNumber,int* TopHitDamageUser) // OK
{
	int TopHitDamage = 0;
...
void CMonster::MonsterDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	this->DropItem(lpObj,lpTarget);

	lpObj->NextActionTime = 500;

	if(OBJECT_RANGE(lpObj->SummonIndex) != 0)
	{
		this->SummonKill(lpObj->SummonIndex);
	}
}

void CMonster::DropItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	int aIndex = this->GetTopHitDamageUser(lpObj);
	...
	if(lpObj->Class >= 505 && lpObj->Class <= 511) // Destler, Vermont, Kato, Galia, Erkanne, Raymond, Jerint
	{
		gImperialGuardian->DropSideBossItem(lpObj,lpTarget);
		return;
	}

	if(lpObj->Class == 526) // Stone Statue
	{
		gImperialGuardian->DropStoneStatueItem(lpObj,lpTarget);
		return;
	}
	...
	if(lpObj->Class == 592 || lpObj->Class == 593 || lpObj->Class == 594 || lpObj->Class == 595 || lpObj->Class == 596) // Undine, Salamander, Sylphid, Gnome, Hellraiser
	{
		return;
	}

	if(lpObj->Class == 611 || lpObj->Class == 612 || lpObj->Class == 613 || lpObj->Class == 614 || lpObj->Class == 615) // Sellihoden, Ukanva, Silla, Normus, Muff
	{
		return;
	}

	if(lpObj->Class == 598 || lpObj->Class == 599 || lpObj->Class == 600 || lpObj->Class == 601 || lpObj->Class == 602) // Fire Tower, Water Tower, Earth Tower, Wind Tower, Darkness Tower
	{
		gArcaBattle->OccupyObelisk(lpObj,lpTarget);
		return;
	}
...
void CMonster::MonsterMultilAttack(LPOBJ lpObj,int skill) // OK
{
	...
	if(lpObj->Class == 161 || lpObj->Class == 181 || lpObj->Class == 189 || lpObj->Class == 197 || lpObj->Class == 267 || lpObj->Class == 275 || lpObj->Class == 338)
	{
		...
		{
			PMSG_MULTI_SKILL_ATTACK* lpInfo = (PMSG_MULTI_SKILL_ATTACK*)(((BYTE*)send)+sizeof(PMSG_MULTI_SKILL_ATTACK_RECV)+(sizeof(PMSG_MULTI_SKILL_ATTACK)*n));
			...
			{
				gAttack->Attack(lpObj,&gObj[bIndex],lpSkill,1,1,0,0,0);
			}
		}
	}
	else
...
void CMonster::MonsterSkillAttack(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
...
void CMonster::ProcessHit(LPOBJ lpObj,int aIndex) // OK
{
	if(lpObj->Attribute == ATTRIBUTE_NONE)
	{
		return;
