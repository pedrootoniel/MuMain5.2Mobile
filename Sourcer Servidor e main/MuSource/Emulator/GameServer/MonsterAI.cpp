...
void CMonsterAI::MonsterAIProc() // OK
{
	...
	gMonsterAIRule->MonsterAIRuleProc();
	...
	for(int n=0;n < MAX_OBJECT_MONSTER;n++)
	{
		if(gObjIsConnected(n) == 0 || gObj[n].Live == 0 || gObj[n].CurrentAI == 0)
		{
			...
			{
				this->MonsterStateMsgProc(n);
			}
		}
		else
		{
			this->UpdateCurrentAIUnit(n);
			this->RunAI(n,gObj[n].Class);
		}
	}
...
bool CMonsterAI::RunAI(int aIndex,int MonsterClass) // OK
{
	...
	this->MonsterStateMsgProc(aIndex);
	...
	CMonsterAIUnitInfo* lpMonsterAIUnitInfo = gMonsterAIUnit->FindAIUnitInfo(lpObj->CurrentAI);
...
void CMonsterAI::MonsterStateMsgProc(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_MONSTER_SEND_MSG;n++)
	{
		...
		{
			this->ProcessStateMsg(&gObj[aIndex],gSMMsg[aIndex][n].MsgCode,gSMMsg[aIndex][n].SendUser,gSMMsg[aIndex][n].SubCode);
...
void CMonsterAI::ProcessStateMsg(LPOBJ lpObj,int code,int aIndex,int SubCode) // OK
{
	...
	{
		case 1:
			gMonster->DropItem(lpObj,&gObj[aIndex]);
			break;
		case 2:
...
bool CMonsterAI::UpdateCurrentAIUnit(int aIndex) // OK
{
	...
	if(gMonsterAIRule->GetCurrentAIUnit(lpObj->Class) == 0 && lpObj->BasicAI != 0)
	{
		lpObj->CurrentAI = lpObj->BasicAI;
	}
	else
	{
		lpObj->CurrentAI = gMonsterAIRule->GetCurrentAIUnit(lpObj->Class);
	}
...
void CMonsterAI::MonsterMove(int aIndex) // OK
{
	...
	if(lpObj->PathCount == 0 || gMonsterAIUtil->CheckMovingCondition(lpObj) == 0)
	{
		lpObj->PathCur = 0;
		lpObj->PathCount = 0;
