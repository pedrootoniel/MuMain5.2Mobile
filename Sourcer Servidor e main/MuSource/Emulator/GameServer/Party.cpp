...
bool CParty::CheckSameParty(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
...
{
	if(gMapManager->GetMapPartyEnable(gObj[aIndex].Map) == 0)
	{
		return;
	}
	...
	if(lpObj->DieRegen != 0 || lpObj->Teleport != 0 || lpTarget->DieRegen != 0 || lpTarget->Teleport != 0)
	{
		return;
	}

	if(gMapManager->GetMapPartyEnable(lpTarget->Map) == 0)
	{
...
void CParty::CGPartyRequestResultRecv(PMSG_PARTY_REQUEST_RESULT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gMapManager->GetMapPartyEnable(lpObj->Map) == 0)
	{
		this->GCPartyResultSend(aIndex,0);
		goto CLEAR_JUMP;
	}
...
void CParty::CGPartyDelMemberRecv(PMSG_PARTY_DEL_MEMBER_RECV* lpMsg,int aIndex) // OK
{
	...
	if(DG_MAP_RANGE(lpTarget->Map) != 0 || IG_MAP_RANGE(lpTarget->Map) != 0)
	{
		return;
	}
