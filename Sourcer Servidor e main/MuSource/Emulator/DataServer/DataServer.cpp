...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	wsprintf(buff,DATASERVER_VERSION);
	...
	if(WSAStartup(MAKEWORD(2,2),&wsa) == 0)
	{
		...
		int DataServerPort = GetPrivateProfileInt("DataServerInfo","DataServerPort",55960,".\\DataServer.ini");

		wsprintf(buff,"DataServer_EX401_%s",DataServerODBC);
...
BOOL InitInstance(HINSTANCE hInstance,int nCmdShow) // OK
{
	...
	int iQueueBarWidths[] = { -1 };

	SendMessage(hWndStatusBar, SB_SETPARTS, 1, (LPARAM)iQueueBarWidths);
