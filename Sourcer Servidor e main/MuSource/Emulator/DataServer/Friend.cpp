...
void CFriend::GDFriendStatusRecv(SDHP_FRIEND_STATUS_RECV* lpMsg,int index) // OK
{
	if(gQueryManager->ExecQuery("SELECT GUID FROM T_CGuid WHERE Name='%s'",lpMsg->Name) == 0 || gQueryManager->Fetch() == SQL_NO_DATA)
	{
		gQueryManager->Close();
	}
	else
	{
		int guid = gQueryManager->GetAsInteger("GUID");

		gQueryManager->Close();

		gQueryManager->ExecQuery("UPDATE T_FriendList SET Del=%d WHERE GUID=%d",((lpMsg->status==0)?1:0),guid);

		gQueryManager->Close();
	}

	this->DGFriendStateSend(lpMsg->Name,((lpMsg->status==0)?1:0));
}
