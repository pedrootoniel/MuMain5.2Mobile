...
{
	LogAdd(LOG_RED,"[SocketUDP] GameServer disconnected [%s] [%s:%d][%d]",this->m_ServerName,this->m_IpAddr,this->m_ServerPort,this->m_ServerCode);
...
void CServerManager::SetServerInfo(char* name,WORD port,WORD code) // OK
{
	...
	LogAdd(LOG_GREEN,"[SocketUDP] GameServer connected [%s] [%s:%d][%d]",this->m_ServerName,this->m_IpAddr,this->m_ServerPort,this->m_ServerCode);
}
