...
bool CSocketManager::Start(WORD port) // OK
{
	...
	LogAdd(LOG_BLACK,"[SocketTCP] Server started at port [%d]",this->m_port);
	return 1;
}
...
bool CSocketManager::CreateListenSocket() // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketTCP] WSASocket() failed with error: %d",WSAGetLastError());
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"[SocketTCP] bind() failed with error: %d",WSAGetLastError());
		return 0;
	}

	if(listen(this->m_listen,5) == SOCKET_ERROR)
	{
		LogAdd(LOG_RED,"[SocketTCP] listen() failed with error: %d",WSAGetLastError());
		return 0;
	}
...
bool CSocketManager::CreateCompletionPort() // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketTCP] socket() failed with error: %d",WSAGetLastError());
		return 0;
	}

	if((this->m_CompletionPort=CreateIoCompletionPort((HANDLE)socket,0,0,0)) == 0)
	{
		LogAdd(LOG_RED,"[SocketTCP] CreateIoCompletionPort() failed with error: %d",GetLastError());
		closesocket(socket);
		return 0;
	}
...
bool CSocketManager::CreateAcceptThread() // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketTCP] CreateThread() failed with error: %d",GetLastError());
		return 0;
	}

	if(SetThreadPriority(this->m_ServerAcceptThread,THREAD_PRIORITY_HIGHEST) == 0)
	{
		LogAdd(LOG_RED,"[SocketTCP] SetThreadPriority() failed with error: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::CreateServerQueue() // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketTCP] CreateSemaphore() failed with error: %d",GetLastError());
		return 0;
	}
...
bool CSocketManager::DataRecv(int index,IO_MAIN_BUFFER* lpIoBuffer) // OK
{
	...
	while(true)
	{
		...
		{
			gLog->Output(LOG_CONNECTION,"[SocketTCP] Protocol header error (Index: %d,Header: %x)",index,lpMsg[count]);
			return 0;
		}

		if(size < 3 || size > MAX_MAIN_PACKET_SIZE)
		{
			gLog->Output(LOG_CONNECTION,"[SocketTCP] Protocol size error (Index: %d,Header: %x,Size: %d,Head: %x)",index,header,size,head);
			return 0;
		}
...
bool CSocketManager::DataSend(int index,BYTE* lpMsg,int size) // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketTCP] Max msg size (Type: 1,Index: %d,Size: %d)",index,size);
		this->m_critical.unlock();
		return 0;
	}
	...
	if(lpIoContext->IoSize > 0)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketTCP] Max msg size (Type: 2,Index: %d,Size: %d)",index,(lpIoContext->IoSideBuffer.size+size));
...
void CSocketManager::Disconnect(int index) // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketTCP] closesocket() failed with error: %d",WSAGetLastError());
		this->m_critical.unlock();
		return;
	}
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	if((++lpClientManager->m_PacketCount) >= MaxPacketPerSecond)
	{
		gLog->Output(LOG_HACK,"[DataRecv] Packets exceeded. [%d][%d] (Index: %d,IpAddress: %s)",index,lpClientManager->m_IpAddr,lpClientManager->m_PacketCount,MaxPacketPerSecond);
		this->Disconnect(index);
		this->m_critical.unlock();
	}
	...
	{
		gLog->Output(LOG_CONNECTION,"[DataRecv] MainBuffer Error (Index: %d,IpAddress: %s)",index,gClientManager[index].m_IpAddr);
		this->Disconnect(index);
		this->m_critical.unlock();
		return;
	}
	...
	if(WSARecv(lpClientManager->m_socket,&lpIoContext->wsabuf,1,&RecvSize,&Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSA_IO_PENDING)
		{
			gLog->Output(LOG_CONNECTION,"[SocketTCP] WSARecv() failed with error: %d",WSAGetLastError());
			this->Disconnect(index);
			this->m_critical.unlock();
			return;
		}
...
int CALLBACK CSocketManager::ServerAcceptCondition(IN LPWSABUF lpCallerId,IN LPWSABUF lpCallerData,IN OUT LPQOS lpSQOS,IN OUT LPQOS lpGQOS,IN LPWSABUF lpCalleeId,OUT LPWSABUF lpCalleeData,OUT GROUP FAR* g,CSocketManager* lpSocketManager) // OK
{
	...
	if(gBlackList->CheckIpAddress(inet_ntoa(SocketAddr->sin_addr)) == 0)
	{
		return CF_REJECT;
	}
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		...
		if(WSARecv(socket,&lpClientManager->m_IoRecvContext->wsabuf,1,&RecvSize,&Flags,&lpClientManager->m_IoRecvContext->overlapped,0) == SOCKET_ERROR)
		{
			if(WSAGetLastError() != WSA_IO_PENDING)
			{
				LogAdd(LOG_RED,"[SocketTCP] WSARecv() failed with error: %d",WSAGetLastError());
...
DWORD WINAPI CSocketManager::ServerWorkerThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		if(GetQueuedCompletionStatus(lpSocketManager->m_CompletionPort,&IoSize,&index,&lpOverlapped,INFINITE) == 0)
		{
			...
			{
				lpSocketManager->m_critical.lock();
				LogAdd(LOG_RED,"[SocketTCP] GetQueuedCompletionStatus() failed with error: %d",GetLastError());
				lpSocketManager->m_critical.unlock();
				return 0;
			}
		}
