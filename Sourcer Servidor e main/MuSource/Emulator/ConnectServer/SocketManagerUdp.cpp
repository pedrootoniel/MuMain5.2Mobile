...
bool CSocketManagerUdp::Start(WORD port) // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketUDP] WSASocket() failed with error: %d",WSAGetLastError());
		this->Clean();
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"[SocketUDP] bind() failed with error: %d",WSAGetLastError());
		this->Clean();
		return 0;
	}
	...
	{
		LogAdd(LOG_RED,"[SocketUDP] CreateThread() failed with error: %d",GetLastError());
		this->Clean();
		return 0;
	}
	...
	LogAdd(LOG_BLACK,"[SocketUDP] Server started at port [%d]",port);
	return 1;
}
...
bool CSocketManagerUdp::Connect(char* IpAddress,WORD port) // OK
{
	...
	if(this->m_SocketAddr.sin_addr.s_addr == INADDR_NONE)
	{
		...
		if(host == 0)
		{
			LogAdd(LOG_RED,"[SocketUDP] gethostbyname() failed with error: %d",WSAGetLastError());
			this->Clean();
			return 0;
		}
...
bool CSocketManagerUdp::DataRecv() // OK
{
	...
	while(true)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketUDP] Protocol header error (Header: %x)",lpMsg[count]);
			memset(this->m_RecvBuff,0,sizeof(this->m_RecvBuff));
			this->m_RecvSize = 0;
			return 0;
		}
		...
		{
			LogAdd(LOG_RED,"[SocketUDP] Protocol size error (Header: %x,Size: %d,Head: %x)",header,size,head);
			memset(this->m_RecvBuff,0,sizeof(this->m_RecvBuff));
			this->m_RecvSize = 0;
			return 0;
		}
...
bool CSocketManagerUdp::DataSend(BYTE* lpMsg,int size) // OK
{
	...
	{
		LogAdd(LOG_RED,"[SocketUDP] Max msg size (Size: %d)",size);
		memset(this->m_SendBuff,0,sizeof(this->m_SendBuff));
		this->m_SendSize = 0;
		return 0;
	}
	...
	if(result == SOCKET_ERROR)
	{
		...
		{
			LogAdd(LOG_RED,"[SocketUDP] sendto() failed with error: %d",WSAGetLastError());
			memset(this->m_SendBuff,0,sizeof(this->m_SendBuff));
			this->m_SendSize = 0;
			return 0;
		}
...
DWORD WINAPI CSocketManagerUdp::ServerRecvThread(CSocketManagerUdp* lpSocketManagerUdp) // OK
{
	while(true)
	{
		...
		if(result == SOCKET_ERROR)
		{
			LogAdd(LOG_RED,"[SocketUDP] recvfrom() failed with error: %d",GetLastError());
