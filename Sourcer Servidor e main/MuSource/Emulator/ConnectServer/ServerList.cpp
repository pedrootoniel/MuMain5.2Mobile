...
void CServerList::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.ServerCode = lpReadFile->GetNumber();

			strcpy_s(info.ServerName,lpReadFile->GetAsString());

			strcpy_s(info.ServerAddress,lpReadFile->GetAsString());

			info.ServerPort = lpReadFile->GetAsNumber();

			info.ServerShow = ((lpReadFile->GetAsNumber()>0)?1:0);
